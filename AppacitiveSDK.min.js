...
c&&"live"!=c&&(c="sandbox");m=c}return m}};c.Appacitive.getAppPrefix=function(e){return c.Appacitive.Session.environment()+"/"+c.Appacitive.appId+"/"+e};c.Appacitive.initialize=function(e){e=e||{};if(!c.Appacitive.Session.initialized){e.masterKey&&0<e.masterKey.length&&c.Appacitive.Session.setMasterKey(e.masterKey);if(!e.apikey||0===e.apikey.length)if(e.masterKey)e.apikey=e.masterKey;else throw Error("apikey is mandatory");if(!e.appId||0===e.appId.length)throw Error("appId is mandatory");c.Appacitive.Session.setApiKey(e.apikey);
...
c=c.toLowerCase();delete k[c];w(c,b);return this};this.has=function(c){return c&&_type.isString(c)&&0!==c.length?k[c]&&!_type.isUndefined(k[c])?!0:!1:!1};this.isNew=function(){return k.__id&&k.__id.length?!1:!0};this.clone=function(){return"object"==this.type?c.Appacitive.Object._create(this.toJSON()):new c.Appacitive.connection._create(this.toJSON())};this.copy=function(c,b){c&&(m(c,k),b&&m(c,f));return this};this.mergeWithPrevious=function(){m(k,f);n._aclFactory&&n._aclFactory.merge();q=[];h={};
l={};g={};return this};var C=function(){m(f,k);n._aclFactory&&n._aclFactory.merge();q=[];h={};l={};g={}};this.rollback=function(){k=p={};C();return this};var D=function(c,b,g,e){if(!c||!_type.isString(c)||0===c.length||0===c.indexOf("__"))return this;c=c.toLowerCase();if(_type.isObject(k[c])||_type.isArray(k[c]))throw Error("Cannot increment/decrement array/object");try{_type.isObject(b)?(e=b,b=g):b=!b||isNaN(Number(b))?g:Number(b)*g,k[c]=isNaN(Number(k[c]))?b:Number(k[c])+b,n.isNew()||(h[c]={value:(h[c]?
h[c].value:0)+b})}catch(f){throw Error("Cannot perform increment/decrement operation");}w(c,e);return n};this.increment=function(c,b,g){return D(c,b,1,g)};this.decrement=function(c,b,g){return D(c,b,-1,g)};this.save=function(){return k.__id?F.apply(this,arguments):G.apply(this,arguments)};var G=function(b){var g=n.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(g=k.__type.toLowerCase());for(var e in k)"$"==e[0]&&delete k[e];k.__revision&&delete k.__revision;"object"==
g&&n._aclFactory&&(e=n._aclFactory.getChanged())&&(k.__acls=e);var h=new c.Appacitive._Request({method:"PUT",type:g,op:"getCreateUrl",args:[this.className,r],data:k,options:b,entity:n,onSuccess:function(e){var m=null;e&&e[g]?(f=m=e[g],k.__id=m.__id,C(),"connection"==n.type&&n.parseConnection(),c.Appacitive.eventManager.fire(n.entityType+"."+g+".created",n,{object:n}),n.created=!0,n.trigger("sync",n,e[g],b),h.promise.fulfill(n)):(c.Appacitive.eventManager.fire(n.entityType+"."+g+".createFailed",n,
{error:e.status}),h.promise.reject(e.status,n))}});return h.send()},F=function(b,g){c.Appacitive.Promise.is(g)||(g=c.Appacitive.Promise.buildPromise(b));var e=z(!0),h;for(h in e)"$"==h[0]&&delete e[h];if(Object.isEmpty(e))g.fulfill(n);else{var m=n.type;h=[n.className,f.__id?f.__id:k.__id,r];k&&(k.__type&&("user"==k.__type.toLowerCase()||"device"==k.__type.toLowerCase()))&&(m=k.__type.toLowerCase(),h.splice(0,1));var l=new c.Appacitive._Request({method:"POST",type:m,op:"getUpdateUrl",args:h,data:e,
options:b,entity:n,onSuccess:function(g){g&&g[m]?(f=g[m],C(),delete n.created,n.trigger("sync",n,g[m],b),c.Appacitive.eventManager.fire(n.entityType+"."+m+"."+k.__id+".updated",n,{object:n}),l.promise.fulfill(n)):(g=g||{},g.status=g.status||{},g.status=v(g.status),c.Appacitive.eventManager.fire(n.entityType+"."+m+"."+k.__id+".updateFailed",n,{object:g.status}),l.promise.reject(g.status,n))}});return l.send()}return g},H=function(b){if(!k.__id)throw Error("Please specify id for get operation");var g=
this.type;k&&(k.__type&&("user"==k.__type.toLowerCase()||"device"==k.__type.toLowerCase()))&&(g=k.__type.toLowerCase());var e=new c.Appacitive._Request({method:"GET",type:g,op:"getGetUrl",args:[this.className,k.__id,r],options:b,entity:n,onSuccess:function(c){c&&c[g]?(f=c[g],m(f,k),n._aclFactory&&n._aclFactory._rollback(),c.connection&&(n.endpoints||n.endpointA&&n.endpointB||n.setupConnection(k.__endpointa,k.__endpointb)),n.trigger("sync",n,c[g],b),e.promise.fulfill(n)):(c=c||{},c.status=c.status||
{},c.status=v(c.status),e.promise.reject(c.status,n))}});return e.send()};this.fetch=function(c){return H.apply(this,[c])};this.del=this.destroy=function(b){b=b||{};var g=b.deleteConnections;_type.isBoolean(b)&&(g=b,b={});b.wait||E(b);if(!k.__id)return(new c.Appacitive.Promise.buildPromise(b)).fulfill();var e=this.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(e=k.__type.toLowerCase());var f=new c.Appacitive._Request({method:"DELETE",type:e,op:"getDeleteUrl",args:[this.className,
k.__id,g],options:b,entity:this,onSuccess:function(c){f.promise.fulfill(c);c&&c.status?(b.wait&&E(b),f.promise.fulfill(c.status)):(c=c||{},c.status=c.status||{},c.status=v(c.status),f.promise.reject(c.status,n))}});return f.send()};"object"==this.type&&(this.destroyWithConnections=function(c){return this.destroy(_extend({deleteConnections:!0},c))})};c.Appacitive.BaseObject.prototype.toString=function(){return JSON.stringify(this.getObject())};c.Appacitive.Events.mixin(c.Appacitive.BaseObject.prototype)})(global);
