...
this.operator=b;this.innerFilters=[];for(var h=0;h<f.length;h+=1){if(!(f[h]instanceof c))throw Error("Invalid filter provided");this.innerFilters.push(f[h])}this.toString=function(){var b=this.operator,c="(";this.innerFilters.forEach(function(f){c=1===c.length?c+(" "+f.toString()):c+String.format(" {0} {1} ",b,f.toString())});return c+=")"}};p.prototype=new c;p.prototype.constructor=p;var k={isEqualTo:"==",notEqualTo:"<>",isGreaterThan:">",isGreaterThanEqualTo:">=",isLessThan:"<",isLessThanEqualTo:"<=",
...
this.getValue=function(){return"number"===this.type||_type.isBoolean(this.value)||_type.isNumber(this.value)?this.value:"object"===this.type&&_type.isDate(this.value)?"datetime('"+b.Date.toISOString(this.value)+"')":"object"==this.type&&this.value instanceof b.GeoCoord?this.value.toString():"'"+this.value.toString()+"'"}},y=function(b,c,g){g||(g=this);g.type=b;g.name=c;g.equalTo=function(b){return new d({field:this.name,fieldType:this.type,value:new t(b),operator:k.isEqualTo})};g.notEqualTo=function(b){return new d({field:this.name,
fieldType:this.type,value:new t(b),operator:k.notEqualTo})};g.greaterThan=function(b){return new d({field:this.name,fieldType:this.type,value:new t(b),operator:k.isGreaterThan})};g.greaterThanEqualTo=function(b){return new d({field:this.name,fieldType:this.type,value:new t(b),operator:k.isGreaterThanEqualTo})};g.lessThan=function(b){return new d({field:this.name,fieldType:this.type,value:new t(b),operator:k.isLessThan})};g.lessThanEqualTo=function(b){return new d({field:this.name,fieldType:this.type,
value:new t(b),operator:k.isLessThanEqualTo})};g.like=function(b){return new d({field:this.name,fieldType:this.type,value:new t("*"+b+"*"),operator:k.like})};g.match=function(b){return new d({field:this.name,fieldType:this.type,value:new t("*"+b+"*"),operator:k.match})};g.startsWith=function(b){return new d({field:this.name,fieldType:this.type,value:new t(b+"*"),operator:k.like})};g.endsWith=function(b){return new d({field:this.name,fieldType:this.type,value:new t("*"+b),operator:k.like})};g.contains=
function(b){return new e({field:this.name,fieldType:this.type,value:b,operator:k.isEqualTo})};g.containedIn=function(b){return new h({field:this.name,fieldType:this.type,value:b,operator:k.containedIn})};g.isNull=function(){return new l({field:this.name,fieldType:this.type,operator:k.isNull})};g.notIn=function(b){return new h({field:this.name,fieldType:this.type,value:b,operator:k.notIn})};g.isNotNull=function(){return new l({field:this.name,fieldType:this.type,operator:k.isNotNull})};g.between=function(b,
c){return new f({field:this.name,fieldType:this.type,val1:new t(b),val2:new t(c),operator:k.between})};g.withinCircle=function(b,c,f){return new n({field:this.name,fieldType:this.type,geoCoord:b,distance:c,unit:f,operator:k.withinCircle})};g.withinPolygon=function(b){return new q({field:this.name,fieldType:this.type,geoCoords:b,operator:k.withinPolygon})}},w=function(b){if(!b||0===b.length)throw Error("Specify field name");this.field=b;y("property",b,this);return this},u=function(b){if(!b||0===b.length)throw Error("Specify field name");
...
Or:function(){return new p(k.or,arguments)},And:function(){return new p(k.and,arguments)},taggedWithOneOrMore:function(b){return new r({tags:b,operator:k.taggedWithOneOrMore})},taggedWithAll:function(b){return new r({tags:b,operator:k.taggedWithAll})}}})(global);
...
c+'"" provided for get "'+b+'"');}return m[b]}};this.tryGet=function(b,c,f){b=this.get(b,f);return void 0!==b?b:c};var B=function(b,c){if(c&&!c.silent){var f=C(b,g[b],m[b]);if(void 0!=f[b]||-1!=E.indexOf(b))k.trigger("change:"+b,k,f[b]||m[b],{}),c.ignoreChange||k.trigger("change",k,c)}},J=function(b){b&&!b.silent&&k.trigger("destroy",k,k.collection,b)};this._triggerError=function(b,c){b.silent||k.trigger("error",k,c,b)};var N=function(b,f,d){if(!b||!_type.isString(b)||0===b.length||0===b.trim().indexOf("$"))return this;
b=b.toLowerCase();try{if(_type.isNullOrUndefined(f))m[b]=null;else if(h(f))m[b]=f;else if(_type.isObject(f))if(-1!==c.indexOf(b))m[b]=f;else throw Error("Property cannot have value as an object");else _type.isArray(f)&&(m[b]=[],f.forEach(function(c){if(h(c))m[b].push(c);else throw Error("Multivalued property cannot have values of property as an object");}));delete w[b];delete u[b];delete z[b];_type.isEqual(m[b],g[b])||(z[b]=!0);b==k.idAttribute&&(k.id=f);return this}catch(e){throw Error("Unable to set "+
b);}};this.set=function(b,c,f){var d,e;if(null==b)return this;null==b||"object"===typeof b?(e=b,f=c):(e={})[b]=c;f||(f={});if(!this._validate(e,f))return!1;this.idAttribute in e&&(this.id=e[this.idAttribute]);l(e);var h=!1;for(d in e)c=e[d],N.apply(this,[d,c,_extend({},f,{ignoreChange:!0})]);if(f&&!f.silent)for(d in e)if(void 0!=C(d,g[d],m[d])||-1!=E.indexOf(d))h=!0,k.trigger("change:"+b,k,m[b],{});h&&this.trigger("change",this,f);return this};this.unset=function(b,c){if(!b||!_type.isString(b)||0===
...
var K=function(){n(g,m);k._aclFactory&&k._aclFactory.merge();l(m);v=[];w={};u={};z={}};this.rollback=function(){m=x={};K();return this};var L=function(b,c,f,d){if(!b||!_type.isString(b)||0===b.length||0===b.indexOf("__"))return this;b=b.toLowerCase();if(_type.isObject(m[b])||_type.isArray(m[b]))throw Error("Cannot increment/decrement array/object");try{_type.isObject(c)?(d=c,c=f):c=!c||isNaN(Number(c))?f:Number(c)*f,m[b]=isNaN(Number(m[b]))?c:Number(m[b])+c,k.isNew()||(w[b]={value:(w[b]?w[b].value:
0)+c})}catch(e){throw Error("Cannot perform increment/decrement operation");}B(b,d);return k};this.increment=function(b,c,f){return L(b,c,1,f)};this.decrement=function(b,c,f){return L(b,c,-1,f)};this._findUnsavedChanges=function(){var b={object:null,isNested:!1};if(this.id)b.object=D(!0);else{var c=k.getObject();delete c.__meta;for(var f in c)"$"==f[0]&&delete c[f];c.__revision&&delete c.__revision;"object"==k.type&&k._aclFactory&&(f=k._aclFactory.getChanged())&&(c.__acls=f);c.__tags&&0==c.__tags.length&&
delete c.__tags;Object.isEmpty(c.__attributes)&&delete c.__attributes;"connection"==k.type&&(c.__endpointa.objectid&&delete c.__endpointa.object,c.__endpointb.objectid&&delete c.__endpointb.object);b.object=c}return b};this.getType=function(){var b=k.type;!m.__type||"user"!=m.__type.toLowerCase()&&"device"!=m.__type.toLowerCase()||(b=m.__type.toLowerCase());return b};this._parseOutput=function(c,f){var d=k.getType(),e;c=c||{};if(f&&f[d]){c&&c.parse&&(f[d]=this.parse(f[d]));var h=this.isNew(),n=!1;
"connection"==k.type&&m.__endpointa&&f[d].__endpointa.label!=m.__endpointa.label&&(n=f[d].__endpointa,f[d].__endpointa=f[d].__endpointb,f[d].__endpointb=n,n=!0);g=b._decode(_extend({__meta:_extend(k.meta,f.__meta)},f[d]));k.id=m[k.idAttribute]=f[d][k.idAttribute];K();h?("connection"==k.type&&(c._batch||(n?(m.__endpointa.object&&f.__bmeta&&(m.__endpointa.object.__meta=f.__bmeta),m.__endpointb.object&&f.__ameta&&(m.__endpointb.object.__meta=f.__ameta)):(m.__endpointa.object&&f.__ameta&&(m.__endpointa.object.__meta=
f.__ameta),m.__endpointb.object&&f.__bmeta&&(m.__endpointb.object.__meta=f.__bmeta))),k.parseConnection(c._batch)),k.trigger("change:__id",k,k.id,{}),b.eventManager.fire(k.entityType+"."+d+".created",k,{object:k}),k.created=!0):b.eventManager.fire(k.entityType+"."+d+"."+k.id+".updated",k,{object:k});c.silent||k.trigger("sync",k,f[d],c)}else f=f||{},f.status=f.status||{},e=F(f.status),k._triggerError(c,new b.Error(e)),k.isNew()?b.eventManager.fire(k.entityType+"."+d+".createFailed",k,{error:e}):b.eventManager.fire(k.entityType+
"."+d+"."+k.id+".updateFailed",k,{object:e});return e};this._save=function(){return this.id?O.apply(this,arguments):P.apply(this,arguments)};var P=function(c){c=c||{};var f=k.getType(),d=k._findUnsavedChanges(),e=new b._Request({method:"PUT",type:f,op:"getCreateUrl",args:[this.className,A],data:d.object,options:c,entity:k,onSuccess:function(b){(b=k._parseOutput(c,b))?e.promise.reject(b,k):e.promise.fulfill(k)}});return e.send()},O=function(c,f){b.Promise.is(f)||(f=b.Promise.buildPromise(c));var d=
k._findUnsavedChanges();c=c||{};if(Object.isEmpty(d.object))f.fulfill(k);else{var e=k.type,h=[k.className,k.id,A];m&&m.__type&&("user"==m.__type.toLowerCase()||"device"==m.__type.toLowerCase())&&(e=m.__type.toLowerCase(),h.splice(0,1));var g=new b._Request({method:"POST",type:e,op:"getUpdateUrl",args:h,data:d.object,options:c,entity:k,onSuccess:function(b){(b=k._parseOutput(c,b))?g.promise.reject(b,k):g.promise.fulfill(k)}});return g.send()}return f},Q=function(c){if(!k.id)throw Error("Please specify id for get operation");
c=c||{};var f=k.getType(),d=new b._Request({method:"GET",type:f,op:"getGetUrl",args:[this.className,k.id,A],options:c,entity:k,onSuccess:function(e){e&&e[f]?(c&&c.parse&&(e[f]=k.parse(e[f])),g=b._decode(_extend({__meta:_extend(k.meta,e.__meta)},e[f])),n(g,m),l(m),k._aclFactory&&k._aclFactory._rollback(),e.connection&&(k.endpoints||k.endpointA&&k.endpointB||k.setupConnection(m.__endpointa,m.__endpointb)),c.silent||k.trigger("sync",k,e[f],c),b.eventManager.fire(k.entityType+"."+f+"."+k.id+".fetched",
k,{object:k}),d.promise.fulfill(k)):(e=e||{},e.status=e.status||{},e.status=F(e.status),k._triggerError(c,new b.Error(e.status)),d.promise.reject(e.status,k))}});return d.send()};this._fetch=function(b){return Q.apply(this,[b])};this.del=this._destroy=function(c){c=c||{};var f=c.deleteConnections;_type.isBoolean(c)&&(f=c,c={});c.wait||J(c);if(!k.id)return(new b.Promise.buildPromise(c)).fulfill();var d=k.getType(),e=new b._Request({method:"DELETE",type:d,op:"getDeleteUrl",args:[this.className,k.id,
