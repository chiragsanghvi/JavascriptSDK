...
(function(g){g=g.Appacitive;g.Date={};var b=function(b){return 10>b?"0"+b:b};g.Date.parseISODate=function(b){try{if(!/^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2})T([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z?$/.exec(b))return new Date(b);var d=b.split("T"),e=d[0].split("-"),g=d[1].split("Z")[0].split(":"),k=g[2].split("."),f=Number(g[0]),p=new Date;p.setUTCFullYear(Number(e[0]));p.setUTCDate(Number(e[2]));p.setUTCMonth(Number(e[1])-1);p.setUTCHours(Number(f));p.setUTCMinutes(Number(g[1]));p.setUTCSeconds(Number(k[0]));
k[1]&&p.setUTCMilliseconds(Number(k[1].substring(0,3)));return p}catch(q){return null}};g.Date.toISOString=function(b){try{return b=b.toISOString(),b=b.replace("Z","0000Z")}catch(d){return null}};g.Date.toISODate=function(c){if(c instanceof Date)return String.format("{0}-{1}-{2}",c.getFullYear(),b(c.getMonth()+1),b(c.getDate()));throw Error("Invalid date provided Appacitive.Date.toISODate method");};g.Date.toISOTime=function(c){var d=function(b){return 10>b?b+"000000":100>b?b+"00000":1E3>b?b+"0000":
1E4>b?b+"000":1E5>b?b+"00":1E6>b?b+"0":b};if(c instanceof Date)return String.format("{0}:{1}:{2}.{3}",b(c.getHours()),b(c.getMinutes()),b(c.getSeconds()),d(c.getMilliseconds()));throw Error("Invalid date provided Appacitive.Date.toISOTime method");};g.Date.parseISOTime=function(b){try{var d=new Date,e=b.split("T");1===e.length&&e.push(e[0]);if(!/^([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z?$/.exec(e[1]))return null;var g=e[1].split("Z")[0].split(":"),k=g[2].split("."),f=Number(g[0]);1<e.length?
(d.setUTCHours(Number(f)),d.setUTCMinutes(Number(g[1])),d.setUTCSeconds(Number(k[0])),k[1]&&d.setUTCMilliseconds(Number(k[1].substring(0,3)))):(d.setHours(Number(f)),d.setMinutes(Number(g[1])),d.setSeconds(Number(k[0])),k[1]&&d.setMilliseconds(Number(k[1].substring(0,3))));return d}catch(p){return null}}})(global);
