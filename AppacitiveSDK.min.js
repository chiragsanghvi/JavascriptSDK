...
k=!0;b.headers.forEach(function(c){"content-type"==c.key.toLowerCase()&&(k=!0,"application/json"==c.value.toLowerCase()||"application/javascript"==c.value.toLowerCase()||"application/json; charset=utf-8"==c.value.toLowerCase()||"application/json; charset=utf-8;"==c.value.toLowerCase())&&(k=!1)});if(k)f=b.data;else if(b.data&&(f=b.data,_type.isObject(b.data)))try{f=JSON.stringify(f)}catch(g){}if(!global.navigator||-1==global.navigator.userAgent.indexOf("MSIE 8")&&-1==global.navigator.userAgent.indexOf("MSIE 9")){var p=
...
this.objectServiceUrl,c,b,d(f))},getDeleteUrl:function(c,b,f){return!0===f?String.format("{0}/{1}/{2}?deleteconnections=true",this.objectServiceUrl,c,b):String.format("{0}/{1}/{2}",this.objectServiceUrl,c,b)},getMultiDeleteUrl:function(c){return String.format("{0}/{1}/bulkdelete",this.objectServiceUrl,c)}};this.connection={connectionServiceUrl:"connection",getGetUrl:function(c,b,f){return String.format("{0}/{1}/{2}?fields={3}",this.connectionServiceUrl,c,b,d(f))},getMultiGetUrl:function(b,e,f){return String.format("{0}/{1}/multiGet/{2}?fields={3}",
this.connectionServiceUrl,b,e,d(f))},getCreateUrl:function(b,e){return String.format("{0}/{1}?fields={2}",this.connectionServiceUrl,b,d(e))},getUpdateUrl:function(b,e,f,k){return k?String.format("{0}/{1}/{2}?fields={3}&revision={4}",this.connectionServiceUrl,b,e,d(f),k):String.format("{0}/{1}/{2}?fields={3}",this.connectionServiceUrl,b,e,d(f))},getDeleteUrl:function(b,e){return String.format("{0}/{1}/{2}",this.connectionServiceUrl,b,e)},getMultiDeleteUrl:function(b){return String.format("{0}/{1}/bulkdelete",
this.connectionServiceUrl,b)},getSearchByArticleUrl:function(b,e,f,d){var h="",h=String.format("{0}/{1}/find/all?label={2}&objectid={3}",this.connectionServiceUrl,b,f,e);if("undefined"!==typeof d&&0<d.length)for(b=0;b<d.length;b+=1)h=h+"&"+d[b];return h},getConnectedArticles:function(b,e,f){var d="",d=String.format("{0}/{1}/{2}/find",this.connectionServiceUrl,b,e);if(f&&f.length&&0<f.length)for(b=0;b<f.length;b+=1)d=0===b?d+("?"+f[b]):d+("&"+f[b]);return d},getInterconnectsUrl:function(){return String.format("{0}/interconnects",
...
this.fileServiceUrl,b,escape(e))},getDownloadUrl:function(b,e){return String.format("{0}/download/{1}?expires={2}",this.fileServiceUrl,b,e)},getDeleteUrl:function(b){return String.format("{0}/delete/{1}",this.fileServiceUrl,b)}};this.query={params:function(b){var e=[];return""===location.search||-1===location.search.indexOf("?")?e:b?(b=b.toLowerCase(),location.search.split("?")[1].split("&").forEach(function(f,d){var h=d.split("="),l=h[1];2<h.length&&h.forEach(function(b,c){0!==b&&1!==b&&(l=l+"="+
h[b])});h[0].toLowerCase()===b&&(e=[h[0],l])}),e):location.search.split("?")[1].split("=")}}}})(global);
(function(b){var d;d=b.Appacitive.runtime.isNode?process.nextTick:setTimeout;var c=function(){if(!(this instanceof c))return new c;this.calls=[]};c.prototype.isResolved=function(){return 1===this.state?!0:!1};c.prototype.isRejected=function(){return 2===this.state?!0:!1};c.prototype.isFulfilled=function(){return 1===this.state||2===this.state?!0:!1};c.prototype.done=function(){var e,f,d=this.state,h=this.value;if(!d)return this;for(;e=this.calls.shift();)if(f=e[0],"function"===typeof e[d]){try{h=
e[d].apply(f,this.value)}catch(l){b.Appacitive.logs.logException({name:l.name,message:l.message,stack:l.stack});if(0==f.calls.length)throw l;f.reject(l)}h instanceof c||c.is(h)?h.then(function(b){f.fulfill(b)},function(b){f.reject(b)}):1===d?f.fulfill(h):f.reject(h)}else 1===d?f.fulfill(h):f.reject(h)};c.prototype.fulfill=function(){if(this.state)return this;this.state=1;this.value=arguments;this.done();return this};c.prototype.resolve=c.prototype.fulfill;c.prototype.reject=function(){if(this.state)return this;
this.state=2;this.reason=this.value=arguments;this.done();return this};c.prototype.then=function(b,f){var k=this,h=new c;this.calls[this.calls.length]=[h,b,f];this.state&&d(function(){k.done()});return h};c.when=function(b){var f=[],k=[],h,l=0,m=new c;if(!b)return m.fulfill(f),m;var n=function(){l+=1;l==h&&(m.state||(0<k.length?m.reject(k,f?f:[]):m.fulfill(f?f:[])))},g=function(h){var l=b[h];l instanceof c||l&&"function"===typeof l.then?d(function(){l.then(function(b){f[h]=b;n()},function(b){k[h]=
b;n()})}):d(function(){try{f[h]=l.call()}catch(b){k[h]=b}n()})};Array.isArray(b)||(b=[b]);h=b.length;for(var p=0;p<h;p+=1)g(p);return m};c.is=function(b){return b instanceof c?!0:!1};c.buildPromise=function(b){var f=new c;_type.isObject(b)&&f.then(b.success,b.error);return f};b.Appacitive.Promise=c})(global);
(function(b){b.Appacitive.eventManager=new function(){function b(){var c=function(){return Math.floor(65536*Math.random()).toString(16)};return c()+c()+"-"+c()+"-"+c()+"-"+c()+"-"+c()+c()+c()}var c={};this.subscribe=function(e,f){if("string"!=typeof e||"function"!=typeof f)throw Error("Incorrect subscription call");"undefined"==typeof c[e]&&(c[e]=[]);var k=b();c[e].push({callback:f,id:k});return k};this.unsubscribe=function(b){if(!b)return!1;var f=-1,d=null,h;for(h in c)for(var l=0;l<c[h].length;l+=
1)if(c[h][l].id==b){f=l;d=h;break}return-1!=f?(c[d].splice(f,1),!0):!1};this.fire=function(b,f,d){if("string"!=typeof b)throw Error("Incorrect fire call");if("undefined"==typeof d||null===d)d={};d.eventName=b;if("undefined"!=typeof c.all)for(var h=0;h<c.all.length;h+=1)c.all[h].callback(f,d);h=function(b,c,f){setTimeout(function(){b(c,f)},0)};if("undefined"!=typeof c[b])for(var l=0;l<c[b].length;l+=1)h(c[b][l].callback,f,d)};this.clearSubscriptions=function(b){if("string"!=typeof b)throw Error("Event Name must be string in EventManager.clearSubscriptions");
...
(function(b){var d={},c=Array.prototype.forEach,e=Object.prototype.hasOwnProperty,f=Array.prototype.slice,k=0,h=function(){return{keys:Object.keys,uniqueId:function(b){var c=++k+"";return b?b+c:c},has:function(b,c){return e.call(b,c)},each:function(b,f,h){if(null!=b)if(c&&b.forEach===c)b.forEach(f,h);else if(b.length===+b.length)for(var e=0,l=b.length;e<l&&f.call(h,b[e],e,b)!==d;e++);else for(e in b)if(this.has(b,e)&&f.call(h,b[e],e,b)===d)break},once:function(b){var c=!1,f;return function(){if(c)return f;
c=!0;f=b.apply(this,arguments);b=null;return f}}}}(),l;l={on:function(b,c,f){if(!n(this,"on",b,[c,f])||!c)return this;this._events||(this._events={});(this._events[b]||(this._events[b]=[])).push({callback:c,context:f,ctx:f||this});return this},once:function(b,c,f){if(!n(this,"once",b,[c,f])||!c)return this;var d=this,e=h.once(function(){d.off(b,e);c.apply(this,arguments)});e._callback=c;return this.on(b,e,f)},off:function(b,c,f){var d,e,l,g,k,m,w,v;if(!this._events||!n(this,"off",b,[c,f]))return this;
if(!b&&!c&&!f)return this._events={},this;g=b?[b]:h.keys(this._events);k=0;for(m=g.length;k<m;k++)if(b=g[k],l=this._events[b]){this._events[b]=d=[];if(c||f)for(w=0,v=l.length;w<v;w++)e=l[w],(c&&c!==e.callback&&c!==e.callback._callback||f&&f!==e.context)&&d.push(e);d.length||delete this._events[b]}return this},trigger:function(b){if(!this._events)return this;var c=f.call(arguments,1);if(!n(this,"trigger",b,c))return this;var d=this._events[b],h=this._events.all;d&&g(d,c);h&&g(h,arguments);return this},
stopListening:function(b,c,f){var d=this._listeners;if(!d)return this;var h=!c&&!f;"object"===typeof c&&(f=this);b&&((d={})[b._listenerId]=b);for(var e in d)d[e].off(c,f,this),h&&delete this._listeners[e];return this}};var m=/\s+/,n=function(b,c,f,d){if(!f)return!0;if("object"===typeof f){for(var h in f)b[c].apply(b,[h,f[h]].concat(d));return!1}if(m.test(f)){f=f.split(m);h=0;for(var e=f.length;h<e;h++)b[c].apply(b,[f[h]].concat(d));return!1}return!0},g=function(b,c){var f,d=-1,h=b.length,e=c[0],l=
c[1],g=c[2];switch(c.length){case 0:for(;++d<h;)(f=b[d]).callback.call(f.ctx);break;case 1:for(;++d<h;)(f=b[d]).callback.call(f.ctx,e);break;case 2:for(;++d<h;)(f=b[d]).callback.call(f.ctx,e,l);break;case 3:for(;++d<h;)(f=b[d]).callback.call(f.ctx,e,l,g);break;default:for(;++d<h;)(f=b[d]).callback.apply(f.ctx,c)}};h.each({listenTo:"on",listenToOnce:"once"},function(b,c){l[c]=function(c,f,d){var e=this._listeners||(this._listeners={}),l=c._listenerId||(c._listenerId=h.uniqueId("l"));e[l]=c;"object"===
typeof f&&(d=this);c[b](f,d,this);return this}});l.bind=l.on;l.unbind=l.off;l.mixin=function(b){h.each("on once off trigger stopListening listenTo listenToOnce bind unbind".split(" "),function(c){b[c]=this[c]},this);return b};b.Appacitive.Events=l})(global);
...
(function(b){var d=b.Appacitive;d.GeoCoord=function(b,c){var f=function(b,c){if(isNaN(b)||isNaN(c))throw Error("Invalid Latitiude or longitiude provided");if(-90>b||90<b)throw Error("Latitude "+b+" should be in range of  -90.0 to 90.");if(-180>c||180<c)throw Error("Latitude "+c+" should be in range of  -180.0 to 180.");},h=function(b){if("string"!==typeof b||0==b.length)return!1;b=b.split(",");if(2!==b.length)return!1;try{return new d.GeoCoord(b[0],b[1])}catch(c){return!1}};if(_type.isString(b)&&
...
function(b){return new f({field:this.name,fieldType:this.type,value:b,operator:g.isEqualTo})};d.between=function(b,c){return new k({field:this.name,fieldType:this.type,val1:new p(b,"number"),val2:new p(c,"number"),operator:g.between})};d.betweenDate=function(b,c){return new k({field:this.name,fieldType:this.type,val1:new q(b),val2:new q(c),operator:g.between})};d.betweenTime=function(b,c){return new k({field:this.name,fieldType:this.type,val1:new r(b),val2:new r(c),operator:g.between})};d.betweenDateTime=
function(b,c){return new k({field:this.name,fieldType:this.type,val1:new t(b),val2:new t(c),operator:g.between})};d.withinCircle=function(b,c,f){return new h({field:this.name,fieldType:this.type,geoCoord:b,distance:c,unit:f,operator:g.withinCircle})};d.withinPolygon=function(b){return new l({field:this.name,fieldType:this.type,geoCoords:b,operator:g.withinPolygon})}},A=function(b){if(!b||0===b.length)throw Error("Specify field name");this.field=b;u("property",b,this);return this},z=function(b){if(!b||
...
this.isAscending=function(){return 1===arguments.length?(p.isAscending(arguments[0]),this):p.isAscending()};this.filter=function(){return 1===arguments.length?(h=arguments[0],this):h};this.freeText=function(){if(1===arguments.length){var b=arguments[0];_type.isString(b)?l=arguments[0]:_type.isArray(b)&&b.length&&(l=arguments[0].join(" "));return this}return l};this.fields=function(){if(1===arguments.length){var b=arguments[0];_type.isString(b)?m=b:_type.isArray(b)&&b.length&&(m=b.join(","));return this}return m};
...
c.forEach(function(c){var h=c.__edge;delete c.__edge;var e=b.Appacitive.Object._create(c,!0);h&&(h.__endpointa=d,h.__endpointb={objectid:c.__id,label:h.__label,type:c.__type},delete h.label,e.connection=b.Appacitive.Connection._create(h,!0));f.push(e)});h.object&&(h.object.children[h.relation]=f);return f};this.fetch=function(e){var n=b.Appacitive.Promise.buildPromise(e);e=this.toRequest(e);e.onSuccess=function(b){h.results=l(b.nodes?b.nodes:[],{objectid:c.objectId,type:d,label:b.parent});h._setPaging(b.paginginfo);
n.fulfill(h.results,b.paginginfo)};e.promise=n;e.entity=this;return b.Appacitive.http.send(e)};return this};b.Appacitive.Queries.ConnectedObjectsQuery.prototype=new e;b.Appacitive.Queries.ConnectedObjectsQuery.prototype.constructor=b.Appacitive.Queries.ConnectedObjectsQuery;b.Appacitive.Queries.GetConnectionsQuery=function(c){c=c||{};if(!c.relation)throw Error("Specify relation for GetConnectionsQuery query");if(!c.objectId)throw Error("Specify objectId for GetConnectionsQuery query");if(!c.label||
...
(function(b){b.Appacitive.BaseObject=function(d,c){var e={};(c=c||{})&&c.parse&&(d=this.parse(d));_type.isObject(this.defaults)&&!c.setSnapShot&&(d=_extend({},this.defaults,d));c&&c.collection&&(this.collection=c.collection);var f={},k={},h={},l=function(c,d){for(var e in c)if(f[e]&&delete f[e],k[e]&&delete k[e],h[e]&&delete h[e],_type.isString(c[e]))d[e]=c[e];else if(c[e]instanceof Date)d[e]=b.Appacitive.Date.toISOString(c[e]);else if(_type.isObject(c[e]))if(c[e]instanceof b.Appacitive.GeoCoord)d[e]=
...
m.setupConnection(g.__endpointa,g.__endpointb)),m.trigger("sync",m,b[d],c),f.promise.fulfill(m)):(b=b||{},b.status=b.status||{},b.status=u(b.status),f.promise.reject(b.status,m))}});return f.send()};this.fetch=function(b){return G.apply(this,[b])};this.del=this.destroy=function(c){c=c||{};var d=c.deleteConnections;_type.isBoolean(c)&&(d=c,c={});c.wait||D(c);if(!g.__id)return(new b.Appacitive.Promise.buildPromise(c)).fulfill();var f=this.type;!g.__type||"user"!=g.__type.toLowerCase()&&"device"!=g.__type.toLowerCase()||
(f=g.__type.toLowerCase());var e=new b.Appacitive._Request({method:"DELETE",type:f,op:"getDeleteUrl",args:[this.className,g.__id,d],callbacks:c,entity:this,onSuccess:function(b){e.promise.fulfill(b);b&&b.status?(c.wait&&D(c),e.promise.fulfill(b.status)):(b=b||{},b.status=b.status||{},b.status=u(b.status),e.promise.reject(b.status,m))}});return e.send()};"object"==this.type&&(this.destroyWithConnections=function(b){return this.destroy(_extend({deleteConnections:!0},b))})};b.Appacitive.BaseObject.prototype.toString=
function(){return JSON.stringify(this.getObject())};b.Appacitive.Events.mixin(b.Appacitive.BaseObject.prototype)})(global);
(function(b){var d=function(){return Math.floor(65536*Math.random()).toString(16)};b.Appacitive.GUID=function(){var b=d()+d()+"-"+d()+"-"+d()+"-"+d()+"-"+d()+d()+d(),e="",f,k,h,l,m,n,g=0,b=b.replace(/\r\n/g,"\n");k="";for(h=0;h<b.length;h++)l=b.charCodeAt(h),128>l?k+=String.fromCharCode(l):(127<l&&2048>l?k+=String.fromCharCode(l>>6|192):(k+=String.fromCharCode(l>>12|224),k+=String.fromCharCode(l>>6&63|128)),k+=String.fromCharCode(l&63|128));for(b=k;g<b.length;)f=b.charCodeAt(g++),k=b.charCodeAt(g++),
h=b.charCodeAt(g++),l=f>>2,f=(f&3)<<4|k>>4,m=(k&15)<<2|h>>6,n=h&63,isNaN(k)?m=n=64:isNaN(h)&&(n=64),e=e+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(l)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(f)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(m)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(n);return e.toString()}})(global);
...
_query:null,toJSON:function(b){return this.models.map(function(c){return c.toJSON(b)})},add:function(b,c){c=c||{};var e,f,k,h,l,m={},n={},g=[],p=c.sort,q;b=_type.isArray(b)?b.slice():[b];e=0;for(f=b.length;e<f;e++){b[e]=this._prepareModel(b[e]);k=b[e];if(!k)throw Error("Can't add an invalid model to a collection");h=k.cid;if(m[h]||this._byCid[h])throw Error("Duplicate cid: can't add the same model to a collection twice");(l=k.id())&&((q=n[l])||(q=this._byId[l]))?(q.copy(k.toJSON(),c.setSnapShot),
q.children=k.children):(n[l]=k,m[h]=k,g.push(k),this._addReference(k,c))}this.models.splice.apply(this.models,[null!=c.at?c.at:this.models.length,0].concat(g));p&&this.comparator&&this.sort({silent:!0});this.length=this.models.length;if(c.silent)return this;e=0;for(f=g.length;e<f;e++)k=g[e],c.index=e,k.trigger("add",k,this,c);return this},remove:function(b,c){var e,f,k,h;c=c||{};b=_type.isArray(b)?b.slice():[b];e=0;for(f=b.length;e<f;e++)if(h=this.getByCid(b[e])||this.get(b[e]))delete this._byId[h.id()],
delete this._byCid[h.cid],k=this.models.indexOf(h),this.models.splice(k,1),this.length--,c.silent||(c.index=k,h.trigger("remove",h,this,c)),this._removeReference(h);return this},push:function(b,c){return this.add(b,_extend({at:this.length},c))},pop:function(b){var c=this.at(this.length-1);this.remove(c,b);return c},unshift:function(b,c){return this.add(b,_extend({at:0},c))},shift:function(b){var c=this.at(0);this.remove(c,b);return c},slice:function(){return Array.prototype.slice.apply(this.models,
...
(function(b){b.Appacitive.File=function(d){d=d||{};this.fileId=d.fileId;this.contentType=d.contentType;this.fileData=d.fileData;var c=this,e=function(d,e,f,g){var h=new b.Appacitive.HttpRequest;h.url=d;h.method="GET";h.description=g;h.onSuccess=e;h.promise=f;h.entity=c;b.Appacitive.http.send(h)},f=function(d,e,f,g,h){var k=new b.Appacitive.HttpRequest;k.url=d;k.method="PUT";k.log=!1;k.description="Upload file";k.data=e;k.headers.push({key:"Content-Type",value:f});k.send().then(g,function(b){h.reject(b,
c)})};this.save=function(b){return this.fileId&&_type.isString(this.fileId)&&0<this.fileId.length?h(b):k(b)};var k=function(d){if(!c.fileData)throw Error("Please specify filedata");if(!c.contentType)try{c.contentType=c.fileData.type}catch(h){}c.contentType&&_type.isString(c.contentType)&&0!==c.contentType.length||(c.contentType="text/plain");var k=b.Appacitive.Promise.buildPromise(d);d=b.Appacitive.config.apiBaseUrl+b.Appacitive.storage.urlFactory.file.getUploadUrl(c.contentType,c.fileId?c.fileId:
"");e(d,function(b){f(b.url,c.fileData,c.contentType,function(){c.fileId=b.id;c.getDownloadUrl().then(function(b){return k.fulfill(b,c)},function(b){return k.reject(b)})},k)},k," Get upload url for file ");return k},h=function(d){if(!c.fileData)throw Error("Please specify filedata");if(!c.contentType)try{c.contentType=c.fileData.type}catch(h){}c.contentType&&_type.isString(c.contentType)&&0!==c.contentType.length||(c.contentType="text/plain");var k=b.Appacitive.Promise.buildPromise(d);d=b.Appacitive.config.apiBaseUrl+
...
var k=f[1].split("Z")[0].split(":"),h=k[2].split("."),l=Number(k[0]);1<f.length?(d.setUTCHours(Number(l)),d.setUTCMinutes(Number(k[1])),d.setUTCSeconds(Number(h[0])),h[1]&&d.setUTCMilliseconds(Number(h[1].substring(0,3)))):(d.setHours(Number(l)),d.setMinutes(Number(k[1])),d.setSeconds(Number(h[0])),h[1]&&d.setMilliseconds(Number(h[1].substring(0,3))));return d}catch(m){return null}}})(global);
(function(b){b.Appacitive.localStorage=new (b.Appacitive.runtime.isBrowser?function(){var d=b.Appacitive.runtime.isBrowser?window.localStorage:{getItem:function(){return null}},c=function(){try{return d.setItem("test","1"),d.removeItem("test"),!0}catch(b){return!1}};this.set=function(e,f){f=f||"";if(!e)return!1;if(_type.isObject(f)||_type.isArray(f))try{f=JSON.stringify(f)}catch(k){}if(!c)return b.Appacitive.Cookies.setCookie(e,f),this;e=b.Appacitive.getAppPrefix(e);d[e]=f;return this};this.get=function(e){if(!e)return null;
c?(e=b.Appacitive.getAppPrefix(e),e=d.getItem(e)):e=b.Appacitive.Cookies.readCookie(e);if(!e)return null;if("{"===e[0])try{e=JSON.parse(e)}catch(f){}return e};this.remove=function(e){if(e)if(c){e=b.Appacitive.getAppPrefix(e);try{delete d[e]}catch(f){}}else b.Appacitive.Cookies.eraseCookie(e)}}:function(){var d=[];this.set=function(c,e){e=e||"";if(!c||_type.isString(c))return!1;c=b.Appacitive.getAppPrefix(c);d[c]=e;return this};this.get=function(c){if(!c||_type.isString(c))return null;c=b.Appacitive.getAppPrefix(c);
