...
(function(b){b.Appacitive.logs=[];b.Appacitive.logs.exceptions=[];b.Appacitive.logs.errors=[];var f=function(c,e){if(25<c.length)if(_type.isObject(b.Appacitive.log)&&_type.isString(b.Appacitive.log[e])){var h=require("domain").create();h.run(function(){try{var h="";c.forEach(function(c){h+=JSON.stringify(c,void 0,2)+"\n"});b.Appacitive.logs.length=0;var f=b.Appacitive.log[e],n=require("fs"),h=h.replace(/\r\n|\r/g,"\n"),p=n.openSync(f,"a");new Buffer(h);n.writeSync(p,h);n.closeSync(p)}catch(k){console.log(k)}});
h.on("error",function(){h.dispose()})}else c.splice(0,1)};b.Appacitive.logs.logRequest=function(c,e,h,l){if(b.Appacitive.log){e=e||{};h=h||{};var m=JSON.parse(c.data),n={status:l,referenceId:h.referenceid,date:(new Date).toISOString(),method:m.m,url:decodeURIComponent(c.url),responseTime:c.timeTakenInMilliseconds,headers:{},request:null,response:e.responseText};c.headers&&c.headers.forEach(function(c){n.headers[c.key]=c.value});c.prevHeaders&&c.prevHeaders.forEach(function(c){n.headers[c.key]=c.value});
...
e.prototype.fulfill;e.prototype.reject=function(){if(this.state)return this;this.state=2;this.reason=this.value=arguments;this.done();return this};e.prototype.then=function(b,g){var h=this,l=new e;this.calls[this.calls.length]=[l,b,g];this.state&&f(function(){h.done()});return l};e.when=function(b){var g=[],h=[],l,m=0,n=new e;if(!b)return n.fulfill(g),n;var p=function(){m+=1;m==l&&(n.state||(0<h.length?n.reject(h,g):n.fulfill(g)))},k=function(l){var m=b[l];m instanceof e||m&&"function"===typeof m.then?
f(function(){m.then(function(b){g[l]=b;p()},function(b){h[l]=b;p()})}):f(function(){try{g[l]=m.call()}catch(b){h[l]=b}p()})};Array.isArray(b)||(b=[b]);l=b.length;for(var q=0;q<l;q+=1)k(q);return n};e.is=function(b){return b instanceof e?!0:!1};e.buildPromise=function(b){var g=new e;_type.isObject(b)&&g.then(b.success,b.error);return g};b.Appacitive.Promise=e})(global);
...
g.returnEdge||g.returnEdge||this.prev)||(this.returnEdge=!1);this.label="";var e=this;_type.isString(g.label)&&0<g.label.length&&(this.label="&label="+g.label);this.toRequest=function(){var c=new b.Appacitive.HttpRequest;c.url=this.toUrl();c.method="get";return c};this.toUrl=function(){return b.Appacitive.config.apiBaseUrl+"connection/"+this.relation+"/"+this.type+"/"+this.objectId+"/find?"+this.getQueryString()+this.label+"&returnEdge="+this.returnEdge};var f=function(c,g){var h=[];c.forEach(function(c){var e=
c.__edge;delete c.__edge;var f=b.Appacitive.Object._create(c,!0);e&&(e.__endpointa=g,e.__endpointb={objectid:c.__id,label:e.__label,type:c.__type},delete e.label,f.connection=b.Appacitive.Connection._create(e,!0));h.push(f)});e.object&&(e.object.children[e.relation]=h);return h};this.fetch=function(c){var p=b.Appacitive.Promise.buildPromise(c);c=this.toRequest();c.onSuccess=function(b){e.results=f(b.nodes?b.nodes:[],{objectid:g.objectId,type:h,label:b.parent});e._setPaging(b.paginginfo);p.fulfill(e.results,
...
b.Appacitive.Queries.GetConnectionsQuery;b.Appacitive.Queries.GetConnectionsBetweenObjectsQuery=function(e,h){e=e||{};delete e.entity;if(!e.objectAId||!_type.isString(e.objectAId)||0===e.objectAId.length)throw Error("Specify valid objectAId for GetConnectionsBetweenObjectsQuery query");if(!e.objectBId||!_type.isString(e.objectBId)||0===e.objectBId.length)throw Error("Specify objectBId for GetConnectionsBetweenObjectsQuery query");e.type&&delete e.type;e.queryType=h||"GetConnectionsBetweenObjectsQuery";
...
if(!e||!e.length)throw Error("Specify ids to project");this.name=c;this.data={ids:e};this.queryType="GraphProjectQuery";if(f){this.data.placeholders=f;for(var m in this.data.placeholders)this.data.placeholders[m]=this.data.placeholders[m]}this.toRequest=function(){var c=new b.Appacitive.HttpRequest;c.url=this.toUrl();c.method="post";c.data=this.data;return c};this.toUrl=function(){return b.Appacitive.config.apiBaseUrl+"search/"+this.name+"/project"};var n=function(c){var e,h;for(h in c)if("status"!==
h){e=c[h];break}var f=function(c,e,h){var g=[];c.forEach(function(c){var l=c.__children;delete c.__children;var k=c.__edge;delete c.__edge;c=b.Appacitive.Object._create(c,!0);c.children={};for(var m in l)c.children[m]=[],c.children[m]=f(l[m].values,l[m].parent,c.id);k&&(k.__endpointa={objectid:h,label:e},k.__endpointb={objectid:c.id(),label:k.__label},delete k.__label,c.connection=b.Appacitive.Connection._create(k,!0));g.push(c)});return g};return f(e.values)};this.fetch=function(c){var e=b.Appacitive.Promise.buildPromise(c);
...
arguments[1]}else throw Error(".attr() called with an incorrect number of arguments. 0, 1, 2 are supported.");}return k.__attributes};var y=function(){if(!k.__attributes)return null;if(!c.__attributes)return k.__attributes;var b=!1,e=JSON.parse(JSON.stringify(c.__attributes)),h;for(h in k.__attributes)null==k.__attributes[h]||void 0==k.__attributes[h]?(e[h]=null,b=!0):k.__attributes[h]!=c.__attributes[h]?(e[h]=k.__attributes[h],b=!0):k.__attributes[h]==c.__attributes[h]&&delete e[h];return b?e:null};
