...
function(){if(0!==b.length){var h=[],f;for(f in g)g.hasOwnProperty(f)&&h.push(g[f]);for(;0<b.length;){var m=b.shift(),q=[];for(f in e)e.hasOwnProperty(f)&&q.push(e[f](m));this.changeRequestForCors(m);c.send(m,h,q)}}};this.onResponse=function(c){console.dir(c)}},f=function(){var c=function(){throw Error("Not Implemented Exception");};this.inOnline=this.send=c;this.onResponse=function(b,e){c()};this.onError=function(b){c()}},e=null,e=global.Appacitive.runtime.isBrowser?XMLHttpRequest:require("xmlhttprequest-with-globalagent").XMLHttpRequest,
...
this)}else g.reject(this)};q.open(b.method,b.url,!0);for(var r=0;r<b.headers.length;r+=1)q.setRequestHeader(b.headers[r].key,b.headers[r].value);global.Appacitive.runtime.isBrowser||q.setRequestHeader("User-Agent","Appacitive-NodeJSSDK");q.send(h);return g}b.data=h;return new c(b)},h=function(){var c=new f;c.isOnline=function(){return!0};var b=function(b,e,h){new g({method:b.method,url:b.url,headers:b.headers,data:b.data,onSuccess:function(g,f){if(g){try{g=JSON.parse(g)}catch(p){}var u=g;if(e.length!=
h.length)throw Error("Callback length and state length mismatch!");for(var v=0;v<e.length;v+=1)e[v].apply({},[u,h[v]]);if("200"<=g.code&&"300">=g.code||g.status&&"200"<=g.status.code&&"300">=g.status.code)c.onResponse(b,g);else g=g||{},g=g.status||g,g.message=g.message||"Bad Request",g.code=g.code||"400",c.onError(b,{responseText:JSON.stringify(g)})}else c.onError(b,{responseText:JSON.stringify({code:"400",message:"Invalid request"})})},onError:function(e){var h={};h.message=e.responseText||"Bad Request";
h.code=e.status||"400";c.onError(b,{responseText:JSON.stringify(h)})}})};c.send=function(c,e,h){if(!global.Appacitive.Session.initialized)throw Error("Initialize Appacitive SDK");_type.isFunction(c.beforeSend)&&c.beforeSend(c);b(c,e,h)};return c};global.Appacitive.http=new function(){var c=new h,e=new b(c),g=!1;this.pause=function(){g=!0};this.unpause=function(){g=!1};this.addProcessor=function(c){var b=Error('Must provide a processor object with either a "pre" function or a "post" function.');if(!c)throw b;
if(!c.pre&&!c.post)throw b;e.addProcessor(c)};this.send=function(b){b.promise=global.Appacitive.Promise.is(b.promise)?b.promise:new global.Appacitive.Promise.buildPromise({error:b.onError});e.enqueueRequest(b);c.isOnline()&&!1===g&&e.notify();return b.promise};this.flush=function(b){b?e.notify():c.isOnline()&&e.notify()};this.onError=function(c,b){var e;if(b&&b.responseText)try{e=JSON.parse(b.responseText)}catch(h){}e=e||{code:b.status,message:b.responseText,referenceid:b.headers.TransactionId};global.Appacitive.logs.logRequest(c,
...
e.prototype.fulfill;e.prototype.reject=function(){if(this.state)return this;this.state=2;this.reason=this.value=arguments;this.done();return this};e.prototype.then=function(b,g){var h=this,l=new e;this.calls[this.calls.length]=[l,b,g];this.state&&f(function(){h.done()});return l};e.when=function(b){var g=[],h=[],l,p=0,n=new e;if(!b)return n.fulfill(g),n;var m=function(){p+=1;p==l&&(n.state||(0<h.length?n.reject(h,g):n.fulfill(g)))},k=function(l){var k=b[l];k instanceof e||k&&"function"===typeof k.then?
f(function(){k.then(function(b){g[l]=b;m()},function(b){h[l]=b;m()})}):f(function(){try{g[l]=k.call()}catch(b){h[l]=b}m()})};Array.isArray(b)||(b=[b]);l=b.length;for(var q=0;q<l;q+=1)k(q);return n};e.is=function(b){return b instanceof e?!0:!1};e.buildPromise=function(b){var g=new e;_type.isObject(b)&&g.then(b.success,b.error);return g};b.Appacitive.Promise=e})(global);
...
b);this.value=b.geoCoord;this.unit=b.unit||"mi";this.distance=b.distance||5;this.toString=function(){return String.format("{0}{1} {2} {3},{4} {5}",this.getFieldType(),this.field.toLowerCase(),this.operator,this.value.getValue(),this.distance,this.unit)}};l.prototype=new c;l.prototype.constructor=h;var p=function(b){b=b||"";if(!b.geoCoords||0===b.geoCoords.length)throw Error("polygon filter needs array of Appacitive.GeoCoord objects as argument");if(3>b.geoCoords.length)throw Error("polygon filter needs atleast 3 Appacitive.GeoCoord objects as arguments");
c.call(this,b);this.value=b.geoCoords;var h=function(b){var c="";b.forEach(function(b){c=0===c.length?b.toString():c+(" | "+b.toString())});return c};this.toString=function(){return String.format("{0}{1} {2} {3}",this.getFieldType(),this.field.toLowerCase(),this.operator,h(this.value))}};p.prototype=new c;p.prototype.constructor=h;var n=function(b){e.call(this);b=b||{};if(!b.tags||_type.isArray(b.tags)||0===b.tags.length)throw Error("Specify valid tags");this.tags=b.tags;this.operator=b.operator;
this.toString=function(){return String.format("{0}('{1}')",this.operator,this.tags.join(","))}};n.prototype=new e;n.prototype.constructor=n;var m=function(b,c){if(!c||!c.length||2>c.length)throw Error("Provide valid or atleast 2 filters");this.operator=b;this.innerFilters=[];for(var h=0;h<c.length;h+=1){if(!(c[h]instanceof e))throw Error("Invalid filter provided");this.innerFilters.push(c[h])}this.toString=function(){var b=this.operator,c="(";this.innerFilters.forEach(function(h){c=1===c.length?c+
(" "+h.toString()):c+String.format(" {0} {1} ",b,h.toString())});return c+=")"}};m.prototype=new e;m.prototype.constructor=m;var k={isEqualTo:"==",isGreaterThan:">",isGreaterThanEqualTo:">=",isLessThan:"<",isLessThanEqualTo:"<=",like:"like",match:"match",between:"between",withinCircle:"within_circle",withinPolygon:"within_polygon",or:"or",and:"and",taggedWithAll:"tagged_with_all",taggedWithOneOrMore:"tagged_with_one_or_more"},q=function(b,c){if(null===b||void 0===b||0===b.length)throw Error("Specify value");
...
(c=arguments[0],this):c};this.isAscending=function(){return 1===arguments.length?(l=void 0===arguments[0]||null==arguments[0]?!1:arguments[0],this):l};this.orderBy(b.orderBy);this.isAscending(b.isAscending)};e.prototype.toString=function(){return this.orderBy()&&0<this.orderBy().length?"orderBy="+this.orderBy()+"&isAsc="+this.isAscending():""};var c=function(c){var h=c||{},l="",p="",n="",m=h.queryType||"BasicQuery",k=new f(c),q=new e(c),r=h.type||h.relation,t=h.relation?"connection":"object",u=this;
h.entity&&(this.entity=h.entity);this.type=function(){return t};this.entityType=function(){return r};this.queryType=function(){return m};this.pageQuery=function(){return k};this.pageNumber=function(){return 1===arguments.length?(k.pageNumber(arguments[0]),this):k.pageNumber()};this.pageSize=function(){return 1===arguments.length?(k.pageSize(arguments[0]),this):k.pageSize()};this.sortQuery=function(){return q};this.orderBy=function(){return 1===arguments.length?(q.orderBy(arguments[0]),this):q.orderBy()};
this.isAscending=function(){return 1===arguments.length?(q.isAscending(arguments[0]),this):q.isAscending()};this.filter=function(){return 1===arguments.length?(l=arguments[0],this):l};this.freeText=function(){if(1===arguments.length){var b=arguments[0];_type.isString(b)?p=arguments[0]:_type.isArray(b)&&b.length&&(p=arguments[0].join(" "));return this}return p};this.fields=function(){if(1===arguments.length){var b=arguments[0];_type.isString(b)?n=b:_type.isArray(b)&&b.length&&(n=b.join(","));return this}return n};
...
b.Appacitive.Queries.GetConnectionsQuery;b.Appacitive.Queries.GetConnectionsBetweenObjectsQuery=function(g,h){g=g||{};delete g.entity;if(!g.objectAId||!_type.isString(g.objectAId)||0===g.objectAId.length)throw Error("Specify valid objectAId for GetConnectionsBetweenObjectsQuery query");if(!g.objectBId||!_type.isString(g.objectBId)||0===g.objectBId.length)throw Error("Specify objectBId for GetConnectionsBetweenObjectsQuery query");g.type&&delete g.type;g.queryType=h||"GetConnectionsBetweenObjectsQuery";
...
"GetConnectionsBetweenObjectsForRelationQuery");h.fetch=function(h){var e=b.Appacitive.Promise.buildPromise(h);h=this.toRequest();h.onSuccess=function(h){e.fulfill(h.connection?b.Appacitive.Connection._create(h.connection,!0,c.entity):null)};h.promise=e;h.entity=this;return b.Appacitive.http.send(h)};return h};b.Appacitive.Queries.InterconnectsQuery=function(g){g=g||{};delete g.entity;if(!g.objectAId||!_type.isString(g.objectAId)||0===g.objectAId.length)throw Error("Specify valid objectAId for InterconnectsQuery query");
if(!(g.objectBIds&&_type.isArray(g.objectBIds)&&0<g.objectBIds.length))throw Error("Specify list of objectBIds for InterconnectsQuery query");g.type&&delete g.type;g.queryType="InterconnectsQuery";c.call(this,g);this.objectAId=g.objectAId;this.objectBIds=g.objectBIds;this.toRequest=function(){var c=new b.Appacitive.HttpRequest;c.url=this.toUrl();c.method="post";c.data={object1id:this.objectAId,object2ids:this.objectBIds};return c};this.toUrl=function(){return b.Appacitive.config.apiBaseUrl+"connection/interconnects?"+
this.getQueryString()};return this};b.Appacitive.Queries.InterconnectsQuery.prototype=new c;b.Appacitive.Queries.InterconnectsQuery.prototype.constructor=b.Appacitive.Queries.InterconnectsQuery;b.Appacitive.Queries.GraphFilterQuery=function(c,h){if(!c||0===c.length)throw Error("Specify name of filter query");this.name=c;this.data={};this.queryType="GraphFilterQuery";if(h){this.data.placeholders=h;for(var l in this.data.placeholders)this.data.placeholders[l]=this.data.placeholders[l]}this.toRequest=
function(){var c=new b.Appacitive.HttpRequest;c.url=this.toUrl();c.method="post";c.data=this.data;return c};this.toUrl=function(){return b.Appacitive.config.apiBaseUrl+"search/"+this.name+"/filter"};this.fetch=function(c){var h=b.Appacitive.Promise.buildPromise(c);c=this.toRequest();c.onSuccess=function(b){h.fulfill(b.ids?b.ids:[])};c.promise=h;c.entity=this;return b.Appacitive.http.send(c)}};b.Appacitive.Queries.GraphProjectQuery=function(c,h,l){if(!c||0===c.length)throw Error("Specify name of project query");
if(!h||!h.length)throw Error("Specify ids to project");this.name=c;this.data={ids:h};this.queryType="GraphProjectQuery";if(l){this.data.placeholders=l;for(var e in this.data.placeholders)this.data.placeholders[e]=this.data.placeholders[e]}this.toRequest=function(){var c=new b.Appacitive.HttpRequest;c.url=this.toUrl();c.method="post";c.data=this.data;return c};this.toUrl=function(){return b.Appacitive.config.apiBaseUrl+"search/"+this.name+"/project"};var f=function(c){var h,l;for(l in c)if("status"!==
l){h=c[l];break}var e=function(c,h,l){var g=[];c.forEach(function(c){var f=c.__children;delete c.__children;var k=c.__edge;delete c.__edge;c=b.Appacitive.Object._create(c,!0);c.children={};for(var p in f)c.children[p]=[],c.children[p]=e(f[p].values,f[p].parent,c.id);k&&(k.__endpointa={objectid:l,label:h},k.__endpointb={objectid:c.id(),label:k.__label},delete k.__label,c.connection=b.Appacitive.Connection._create(k,!0));g.push(c)});return g};return e(h.values)};this.fetch=function(c){var h=b.Appacitive.Promise.buildPromise(c);
c=this.toRequest();c.onSuccess=function(b){h.fulfill(f(b))};c.promise=h;c.entity=this;return b.Appacitive.http.send(c)}}})(global);
...
{});k.__attributes[arguments[0]]=arguments[1]}else throw Error(".attr() called with an incorrect number of arguments. 0, 1, 2 are supported.");}return k.__attributes};var y=function(){if(!k.__attributes)return null;if(!c.__attributes)return k.__attributes;var b=!1,h=JSON.parse(JSON.stringify(c.__attributes)),e;for(e in k.__attributes)null==k.__attributes[e]||void 0==k.__attributes[e]?(h[e]=null,b=!0):k.__attributes[e]!=c.__attributes[e]?(h[e]=k.__attributes[e],b=!0):k.__attributes[e]==c.__attributes[e]&&
delete h[e];return b?h:null};this.getChangedAttributes=y;this.aggregate=function(){var b={},c;for(c in k){if(!k.hasOwnProperty(c))return;"$"==c[0]&&(b[c.substring(1)]=k[c])}if(0===arguments.length)return b;if(1==arguments.length)return b[arguments[0]];throw Error(".aggregates() called with an incorrect number of arguments. 0, and 1 are supported.");};this.tags=function(){return k.__tags?k.__tags:[]};this.addTag=function(b){if(!b||!_type.isString(b)||!b.length)return this;k.__tags||(k.__tags=[]);k.__tags.push(b);
k.__tags=Array.distinct(k.__tags);if(!q||!q.length)return this;b=q.indexOf(b);-1!=b&&q.splice(b,1);return this};this.removeTag=function(b){if(!b||!_type.isString(b)||!b.length)return this;q.push(b);q=Array.distinct(q);if(!k.__tags||!k.__tags.length)return this;b=k.__tags.indexOf(b);-1!=b&&k.__tags.splice(b,1);return this};var x=function(){if(!k.__tags)return[];if(!c.__tags)return k.__tags;var b=[];k.__tags.forEach(function(h){-1==c.__tags.indexOf(h)&&b.push(h)});return 0<b.length?b:null};this.getChangedTags=
x;this.getRemovedTags=function(){return _removetags};var w=function(b,c,e){if(!b||!_type.isString(b)||0===b.length||0==b.trim().indexOf("__")||0===b.trim().indexOf("$")||void 0==c||null==c)return this;b=b.toLowerCase();try{var f=function(c){if(!_type.isString(c))if(_type.isNumber(c)||_type.isBoolean(c))c+="";else throw Error("Multivalued property cannot have values of property as an object");if(k[b]&&_type.isArray(k[b])){var f=k[b].indexOf(c);"additems"==e?k[b].push(c):-1==f&&"adduniqueitems"==e?
k[b].push(c):"removeitems"==e&&k[b].removeAll(c)}else"removeitems"!=e&&(k[b]=[c]);h[b]||(h[b]={additems:[],adduniqueitems:[],removeitems:[]});h[b][e].push(c)};_type.isArray(c)?c.forEach(function(b){f(b)}):f(c);return n}catch(l){throw Error("Unable to add item to "+b);}};this.add=function(b,c){return w.apply(this,[b,c,"additems"])};this.addUnique=function(b,c){return w.apply(this,[b,c,"adduniqueitems"])};this.remove=function(b,c){return w.apply(this,[b,c,"removeitems"])};var z=function(b){var e=!1,
f=JSON.parse(JSON.stringify(c)),p;for(p in f)"$"==p[0]&&delete f[p];for(var m in k)null==k[m]||void 0==k[m]?(f[m]=null,e=!0):k[m]!=c[m]?"__tags"==m||"__attributes"==m?delete f[m]:_type.isArray(k[m])?h[m]&&!l[m]?(f[m]=h[m],e=!0):k[m].equals(c[m])?delete f[m]:(f[m]=k[m],e=!0):(f[m]=g[m]&&!l[m]?{incrementby:g[m].value}:k[m],e=!0):k[m]==c[m]&&delete f[m];try{t.forEach(function(b){f[b]&&delete f[b]})}catch(r){}p=x();b?(p&&(f.__addtags=p,e=!0),q&&0<q.length&&(f.__removetags=q,e=!0)):p&&(f.__addtags=p,e=
!0);(b=y())?(f.__attributes=b,e=!0):delete f.__attributes;"object"==n.type&&(b=n._aclFactory.getChanged())&&(f.__acls=b);return e&&!Object.isEmpty(f)?f:!1};this.changed=function(){return this.isNew()?this.toJSON():z()};this.hasChanged=function(){if(this.isNew())return!0;var b=z(!0);return 0===arguments.length?Object.isEmpty(b)?!1:!0:1==arguments.length&&_type.isString(arguments[0])&&0<arguments[0].length&&b&&b[arguments[0]]?!0:!1};this.changedAttributes=function(){if(this.isNew())return this.toJSON();
var b=z(!0);if(0===arguments.length)return b;if(1==arguments.length&&_type.isArray(arguments[0])&&arguments[0].length){var c={};arguments[0].forEach(function(h){b[h]&&c.push(b[h])});return c}return!1};this.previous=function(){return this.isNew()?null:1==arguments.length&&_type.isString(arguments[0])&&arguments[0].length?c[arguments[0]]:null};this.previousAttributes=function(){return this.isNew()?null:c};this.fields=function(){if(1==arguments.length){var b=arguments[0];_type.isString(b)?r=b:_type.isArray(b)&&
(r=b.join(","));return this}return r};var A={integer:function(b){if(b){var c=parseInt(b);if(!isNaN(c))return c}return b},decimal:function(b){if(b){var c=parseFloat(b);if(!isNaN(c))return c}return b},"boolean":function(b){return void 0!==b&&null!==b&&("true"===b.toString().toLowerCase()||!0===b||0<b)?!0:!1},date:function(c){if(c){var h=b.Appacitive.Date.parseISODate(c);if(h)return h}return c},datetime:function(c){if(c){var h=b.Appacitive.Date.parseISODate(c);if(h)return h}return c},time:function(c){if(c){var h=
b.Appacitive.Date.parseISOTime(c);if(h)return h}return c},string:function(b){return b?b.toString():b},geocode:function(c){if(!_type.isString(c)||0==c.trim().length)return!1;c=c.split(",");return 2!==c.length?!1:new b.Appacitive.GeoCoord(c[0],c[1])}};this.get=function(b,c){if(b){if(c&&A[c.toLowerCase()]){if(A[c.toLowerCase()])return A[c.toLowerCase()](k[b]);throw Error('Invalid cast-type "'+c+'"" provided for get "'+b+'"');}return k[b]}};this.tryGet=function(b,c,h){b=this.get(b,h);return void 0!==
...
b;else if(_type.isNumber(b)||_type.isBoolean(b))this[c]=b+"";else throw Error("Multivalued property cannot have values of property as an object");return this});delete g[e];delete h[e];delete l[e];k[e]!==c[e]&&(_type.isArray(k[e])&&!k[e].equals(c[e])||_type.isString(k[e])||_type.isObject(k[e]))&&(l[e]=!0);return this}catch(n){throw Error("Unable to set "+e);}};this.unset=function(b){if(!b||!_type.isString(b)||0===b.length||0===b.indexOf("__"))return this;try{delete k[b]}catch(c){}return this};this.has=
function(b){return b&&_type.isString(b)&&0!==b.length?k[b]&&!_type.isUndefined(k[b])?!0:!1:!1};this.isNew=function(){return k.__id&&k.__id.length?!1:!0};this.clone=function(){return"object"==this.type?b.Appacitive.Object._create(this.toJSON()):new b.Appacitive.connection._create(this.toJSON())};this.copy=function(b,h){b&&(p(b,k),h&&p(b,c));return this};this.mergeWithPrevious=function(){p(k,c);q=[];g={};h={};l={};return this};var B=function(){p(c,k);q=[];g={};h={};l={}};this.rollback=function(){k=
m={};B();return this};var C=function(b,c,h){if(!b||!_type.isString(b)||0===b.length||0===b.indexOf("__"))return this;b=b.toLowerCase();if(_type.isObject(k[b])||_type.isArray(k[b]))throw Error("Cannot increment/decrement array/object");try{return c=!c||isNaN(Number(c))?h:Number(c)*h,k[b]=isNaN(Number(k[b]))?c:Number(k[b])+c,n.isNew()||(g[b]={value:(g[b]?g[b].value:0)+c}),n}catch(e){throw Error("Cannot perform increment/decrement operation");}};this.increment=function(b,c){return C(b,c,1)};this.decrement=
function(b,c){return C(b,c,-1)};this.save=function(){return k.__id?E.apply(this,arguments):D.apply(this,arguments)};var D=function(h){var e=n.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(e=k.__type.toLowerCase());for(var f in k)"$"==f[0]&&delete k[f];k.__revision&&delete k.__revision;"object"==e&&(f=n._aclFactory.getChanged())&&(k.__acls=f);var l=new b.Appacitive._Request({method:"PUT",type:e,op:"getCreateUrl",args:[this.className,r],data:k,callbacks:h,entity:n,
onSuccess:function(h){var f=null;h&&(h.object||h.connection||h.user||h.device)&&(f=h.object||h.connection||h.user||h.device);h&&f?(c=f,k.__id=f.__id,B(),"connection"==n.type?n.parseConnection():n._aclFactory.merge(),b.Appacitive.eventManager.fire(n.entityType+"."+e+".created",n,{object:n}),n.created=!0,l.promise.fulfill(n)):(b.Appacitive.eventManager.fire(n.entityType+"."+e+".createFailed",n,{error:h.status}),l.promise.reject(h.status,n))}});return l.send()},E=function(h,e){b.Appacitive.Promise.is(e)||
(e=b.Appacitive.Promise.buildPromise(h));(function(h){var f=z(!0),l;for(l in f)"$"==l[0]&&delete f[l];if(Object.isEmpty(f))e.fulfill(n);else{l=r;var g=new b.Appacitive.HttpRequest,p=b.Appacitive.config.apiBaseUrl+b.Appacitive.storage.urlFactory[n.type].getUpdateUrl(k.__type||k.__relationtype,c.__id?c.__id:k.__id,l,h),m=n.type;k&&(k.__type&&("user"==k.__type.toLowerCase()||"device"==k.__type.toLowerCase()))&&(m=k.__type.toLowerCase(),p=b.Appacitive.config.apiBaseUrl+b.Appacitive.storage.urlFactory[k.__type.toLowerCase()].getUpdateUrl(c.__id,
l,h));g.url=p;g.method="post";g.data=f;g.onSuccess=function(h){h&&h[m]?(c=h[m],n._aclFactory.merge(),B(),delete n.created,b.Appacitive.eventManager.fire(n.entityType+"."+m+"."+k.__id+".updated",n,{object:n}),e.fulfill(n)):(b.Appacitive.eventManager.fire(n.entityType+"."+m+"."+k.__id+".updateFailed",n,{object:h.status}),e.reject(h.status,n))};g.onError=function(b){b=v(b);e.reject(b,n)};b.Appacitive.http.send(g)}})();return e},F=function(h){if(!k.__id)throw Error("Please specify id for get operation");
var e=this.type;k&&(k.__type&&("user"==k.__type.toLowerCase()||"device"==k.__type.toLowerCase()))&&(e=k.__type.toLowerCase());var f=new b.Appacitive._Request({method:"GET",type:e,op:"getGetUrl",args:[this.className,k.__id,r],callbacks:h,entity:n,onSuccess:function(b){b&&b[e]?(c=b[e],p(c,k),b.connection&&(n.endpoints||n.endpointA&&n.endpointB||n.setupConnection(k.__endpointa,k.__endpointb)),f.promise.fulfill(n)):(b=b||{},b.status=b.status||{},b.status=v(b.status),f.promise.reject(b.status,n))}});return f.send()};
this.fetch=function(b){return F.apply(this,[b])};this.del=this.destroy=function(c,h){_type.isBoolean(c)?(h=c,c=null):_type.isBoolean(h)||(h=!1);if(!k.__id)return(new b.Appacitive.Promise.buildPromise(c)).fulfill();var e=this.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(e=k.__type.toLowerCase());var f=new b.Appacitive._Request({method:"DELETE",type:e,op:"getDeleteUrl",args:[this.className,k.__id,h],callbacks:c,entity:this,onSuccess:function(b){f.promise.fulfill(b)}});
return f.send()}};b.Appacitive.BaseObject.prototype.toString=function(){return JSON.stringify(this.getObject())}})(global);
(function(b){var f=function(){return Math.floor(65536*Math.random()).toString(16)};b.Appacitive.GUID=function(){var b=f()+f()+"-"+f()+"-"+f()+"-"+f()+"-"+f()+f()+f(),c="",g,h,l,p,n,m,k=0,b=b.replace(/\r\n/g,"\n");h="";for(l=0;l<b.length;l++)p=b.charCodeAt(l),128>p?h+=String.fromCharCode(p):(127<p&&2048>p?h+=String.fromCharCode(p>>6|192):(h+=String.fromCharCode(p>>12|224),h+=String.fromCharCode(p>>6&63|128)),h+=String.fromCharCode(p&63|128));for(b=h;k<b.length;)g=b.charCodeAt(k++),h=b.charCodeAt(k++),
l=b.charCodeAt(k++),p=g>>2,g=(g&3)<<4|h>>4,n=(h&15)<<2|l>>6,m=l&63,isNaN(h)?n=m=64:isNaN(l)&&(m=64),c=c+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(p)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(g)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(n)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(m);return c.toString()}})(global);
(function(b){var f=["allow","deny"],e=["create","read","update","delete","manageaccess"],c=function(b){var c=[];b.forEach(function(b){_type.isString(b)&&-1!=e.indexOf(b.toLowerCase())&&c.push(b.toLowerCase())});return c};b.Appacitive._Acl=function(g,h){var l=g||[];_type.isArray(l)||(l=[]);var p={};h&&(p=JSON.parse(JSON.stringify(l)));this.acls=l=JSON.parse(JSON.stringify(l));var n=[],m=function(h,e,g,k){if(!g)throw Error("Specify valid user or usergroup");g instanceof b.Appacitive.Object&&("user"==
g.typeName&&!g.isNew())&&(g=g.id());var p=l.filter(function(b){return b.sid==g&&b.type==e});p&&0!=p.length?p=p[0]:(p={sid:g,type:e,deny:[],allow:[]},l.push(p));p.allow||(p.allow=[]);p.deny||(p.deny=[]);k=c(k);var m=n.filter(function(b){return b.sid==g&&b.type==e});m&&0!=m.length?m=m[0]:(m={sid:g,type:e},n.push(m));k.forEach(function(b){for(var c=0;c<f.length;c+=1){var e=p[f[c]].indexOf(b);if(-1!=e){p[f[c]].splice(e,1);break}}"inherit"!=h&&p[h].push(b);m[b]=h});return this},k=function(b,c,h,e){if(!h)throw Error("Please provide valid id or name for setting acls");
if(!e)throw Error("Please provide valid access permissions for setting acls");_type.isArray(e)||(e=[e]);_type.isArray(h)?h.forEach(function(h){m(b,c,h,e)}):m(b,c,h,e);return this},q=function(){l.allowUser=function(b,c){return k.apply(this,["allow","user",b,c])};l.allowGroup=function(b,c){return k.apply(this,["allow","usergroup",b,c])};l.denyUser=function(b,c){return k.apply(this,["deny","user",b,c])};l.denyGroup=function(b,c){return k.apply(this,["deny","usergroup",b,c])};l.resetUser=function(b,c){return k.apply(this,
...
["deny","usergroup",["anonymous"],b])};l.resetLoggedIn=function(b){return k.apply(this,["inherit","usergroup",["anonymous"],b])}};this._rollback=function(){n=[];l=JSON.parse(JSON.stringify(p));q();return this};this.getChanged=function(){var b=[];n.forEach(function(c){var h={sid:c.sid,type:c.type,allow:[],deny:[],inherit:[]};e.forEach(function(b){c[b]&&h[c[b]].push(b)});f.forEach(function(b){0==h[b].length&&delete h[b]});0==h.inherit.length&&delete h.inherit;b.push(h)});return 0==b.length?null:b};
this.merge=function(){n=[];return this};q();return this};b.Appacitive.Group=new function(){var c=function(c,e,f){if(!e||!_type.isString(e)||0===e.length)throw Error("Please specify valid groupname");_type.isArray(f)||(f=[f]);var g={};g[c]=[];f.forEach(function(e){e&&(e instanceof b.Appacitive.Object&&"user"==e.typeName&&!e.isNew()?g[c].push(e.id()):g[c].push(e))});if(0==g[c].length)throw Error("Please specify valid members as second argument");var m=new b.Appacitive._Request({method:"POST",type:"usergroup",
op:"getUpdateUrl",args:[e],data:g,entity:this,onSuccess:function(b){m.promise.fulfill(b)}});return m.send()};this.addMembers=function(b,e){return c("add",b,e)};this.removeMembers=function(b,e){return c("remove",b,e)}}})(global);
(function(b){b.Appacitive.Object=function(c,e){c=c||{};this.className&&(c.__type=this.className);_type.isString(c)&&(c={__type:c});if(!c.__type)throw Error("Cannot set object without __type");b.Appacitive.BaseObject.call(this,c,e);this.type="object";this.getObject=this.getObject;this.children={};this.toJSON=function(b){if(b){var c=function(b){var e=[];b.forEach(function(b){var h=b.getObject();if(b.children&&!Object.isEmpty(b.children)){h.children={};for(var f in b.children)h.children[f]=c(b.children[f])}b.connection&&
(h.__connection=b.connection.toJSON());e.push(h)});return e};return c([this])[0]}return this.getObject()};this.typeName=c.__type;this._aclFactory=new Appacitive._Acl(c.__acls,e);this.acls=this._aclFactory.acls;return this};b.Appacitive.Object.prototype=new b.Appacitive.BaseObject;b.Appacitive.Object.prototype.constructor=b.Appacitive.Object;b.Appacitive.Object.extend=function(c,e,h){if(!_type.isString(c))throw Error("Appacitive.Object.extend's first argument should be the type-name.");var l=null;
e=e||{};e.className=c;l=b.Appacitive._extend(b.Appacitive.Object,e,h);delete l.extend;l.className=c;return f[c]=l};var f={};b.Appacitive.Object._create=function(c,e,h){if(this.className)h=this;else if(!h){h=c.__type;if(!_type.isString(h))throw"_getClass requires a string argument.";var l=f[h];l||(l=b.Appacitive.Object.extend(h),f[h]=l);h=l}return!0==e?(new h(c)).copy(c,e):(new h(c)).copy(c)};var e=function(c,e){var h=[];c.forEach(function(c){c=b.Appacitive.Object._create(c,!0,e);h.push(c)});return h};
...
var h=f[1].split("Z")[0].split(":"),l=h[2].split("."),p=Number(h[0]);1<f.length?(c.setUTCHours(Number(p)),c.setUTCMinutes(Number(h[1])),c.setUTCSeconds(Number(l[0])),l[1]&&c.setUTCMilliseconds(Number(l[1].substring(0,3)))):(c.setHours(Number(p)),c.setMinutes(Number(h[1])),c.setSeconds(Number(l[0])),l[1]&&c.setMilliseconds(Number(l[1].substring(0,3))));return c}catch(n){return null}}})(global);
