...
b.pagenumber,this.results.pageSize=b.pagesize,b.pagenumber*b.pagesize>=b.totalrecords&&(this.results.isLastPage=!0))};l.prototype._parse=function(b,e,h){b||(b=[]);var f="object"===this.type()?"Object":"Connection";return c[f]._parseResult(b,h.entity,e)};l.prototype.collection=function(b,e){e=e||{};f.isObject(b)&&(e=b,b=null);b||(b=this.results?this.results:[]);var h=options.entity;!h&&0<b.length&&b[0]instanceof c.BaseObject&&(h="object"==b[0].type?"Object":"Connection",h=c[h]._getClass(b[0].className));
h||(h="object"===_etype?"Object":"Connection",h=c[h]._getClass(this[h]));return new c.Collection(b,g(e,{model:h,query:this}))};l.prototype.fetchNext=function(b){var c=this.pageNumber();if(!c||0>=c)c=1;this.pageNumber(++c);return this.fetch(b)};l.prototype.fetchPrev=function(b){var c=this.pageNumber();--c;if(!c||0>=c)c=1;this.pageNumber(c);return this.fetch(b)};l.prototype.count=function(b){var e=c.Promise.buildPromise(b);b=this.toRequest(b);b.onSuccess=function(b){b=b||{};b=b.paginginfo;var c=0,c=
b?b.totalrecords:0;e.fulfill(c)};b.promise=e;b.entity=this;return c.http.send(b)};var k=function(d){var g=d||{};b.call(this,d);e.call(this,d);var h="",m="",k="",u=g.queryType||"BasicQuery",t=g.type||g.relation,r=g.relation?"connection":"object",w=this;g.entity&&(this.entity=g.entity);this.type=function(){return r};this.entityType=function(){return t};this.queryType=function(){return u};this.filter=function(){return 1===arguments.length?(h=arguments[0],this):h};this.freeText=function(){if(1===arguments.length){var b=
arguments[0];f.isString(b)?m=arguments[0]:f.isArray(b)&&b.length&&(m=arguments[0].join(" "));return this}return m};this.fields=function(){if(1===arguments.length){var b=arguments[0];f.isString(b)?k=b:f.isArray(b)&&b.length&&(k=b.join(","));return this}return k};this.filter(g.filter||"");this.freeText(g.freeText||"");this.fields(g.fields||"");this.setFilter=function(b){this.filter(b)};this.setFreeText=function(b){this.freeText(b)};this.setFields=function(b){this.fields(b)};this.extendOptions=function(c){for(var d in c)g[d]=
c[d];b.call(this,g);e.call(this,g);return this};this.getQueryString=function(){var b=this.pageString(),c=this.sortString();c&&(b+="&"+c);this.filter()&&(c=encodeURIComponent(this.filter().toString()),0<c.trim().length&&(b+="&query="+c));this.freeText()&&0<this.freeText().trim().length&&(b+="&freetext="+encodeURIComponent(this.freeText())+"&language=en");this.fields()&&0<this.fields().trim().length&&(b+="&fields="+this.fields());return b};this.toUrl=function(){return{url:c.config.apiBaseUrl+r+"/"+
t+"/find/all?"+this.getQueryString(),description:"FindAll "+t+" "+r+"s"}};this.toRequest=function(b){var d=new c.HttpRequest,h=this.toUrl();d.url=h.url;d.options=b;d.description=h.description;d.method="get";return d};this.getOptions=function(){var b={},c;for(c in this)this.hasOwnProperty(c)&&!f.isFunction(this[c])&&(b[c]=this[c]);return b};this.fetch=function(b){var d=c.Promise.buildPromise(b);b=this.toRequest(b);b.onSuccess=function(b){w.results=w._parse(b[r+"s"],b.__meta,g);w._setPaging(b.paginginfo);
d.fulfill(w.results,b.paginginfo)};b.promise=d;b.entity=this;return c.http.send(b)};this.fetch=function(b){var d=c.Promise.buildPromise(b);b=this.toRequest(b);b.onSuccess=function(b){w.results=w._parse(b[r+"s"],b.__meta,g);w._setPaging(b.paginginfo);d.fulfill(w.results,b.paginginfo)};b.promise=d;b.entity=this;return c.http.send(b)}};g(k.prototype,l.prototype,e.prototype,b.prototype);c.Queries.FindAllQuery=function(b){b=b||{};if(!b.type&&!b.relation)throw Error("Specify either type or relation for basic filter query");
...
(h=d,h.returnObjects=d.returnObjects||h.returnObjects,g=d.placeholders||{},d=h.name);if(!d||0===d.length)throw Error("Specify name of filter query");h=h||{};this.returnObjects=h.returnObjects;b.call(this,h);e.call(this,h);this.name=d;this.data={};this.queryType="GraphQuery";var m=this;if(g){this.data.placeholders=g;for(var k in this.data.placeholders)this.data.placeholders[k]=this.data.placeholders[k]}this.type=function(){return"object"};this.toRequest=function(b){var d=new c.HttpRequest,h=this.toUrl();
d.url=h.url;d.options=b;d.description=h.description;d.method="post";d.data=this.data;return d};this.getQueryString=function(){var b=[];this.isPageDefault||b.push(this.pageString());this.isSortSet&&b.push(this.sortString());return b.join("&")};this.toUrl=function(){return{url:c.config.apiBaseUrl+"search/"+this.name+"/"+(this.returnObjects?"graphquery":"filter")+"?"+this.getQueryString(),description:"Filter Query with name "+this.name}};this.fetch=function(b){var d=c.Promise.buildPromise(b);b=this.toRequest(b);
b.onSuccess=function(b){var c;m.returnObjects?(m.results=m._parse(b.objects,b.__meta,h),c=m.results):this.results=c=b.ids?b.ids:[];m.isPageDefault||m._setPaging(b.paginginfo);d.fulfill(c,b.paginginfo)};b.promise=d;b.entity=this;return c.http.send(b)}};g(c.Queries.GraphQuery.prototype,l.prototype,e.prototype,b.prototype);c.Queries.GraphAPI=function(b,e,h){if(!b||0===b.length)throw Error("Specify name of project query");if(!e||!e.length)throw Error("Specify ids to project");this.name=b;this.data={ids:e};
this.queryType="GraphApi";var m=this;if(h){this.data.placeholders=h;for(var k in this.data.placeholders)this.data.placeholders[k]=this.data.placeholders[k]}this.toRequest=function(b){var d=new c.HttpRequest,h=this.toUrl();d.url=h.url;d.description=h.description;d.method="post";d.data=this.data;d.options=b;return d};this.toUrl=function(){return{url:c.config.apiBaseUrl+"search/"+this.name+"/project",description:"Project Query with name "+this.name}};var u=function(b){var d,h;for(h in b)if("status"!==
h){d=b[h];break}var e=function(b,d,h,f,m){var u=[];b.forEach(function(b){var r=b.__children;delete b.__children;var k=b.__edge;delete b.__edge;b=c.Object._create(g({__meta:f},b),!0);b.children={};for(var t in r)b.children[t]=[],b.children[t]=e(r[t].values,r[t].parent,b.id,r[t].__nodemeta,r[t].__edgemeta);k&&(k.__endpointa={objectid:h,label:d},k.__endpointb={objectid:b.id,label:k.__label},delete k.__label,b.connection=c.Connection._create(g({__meta:m},k),!0));u.push(b)});return u};return e(d.values,
null,null,d.__nodemeta)};this.collection=function(b,d){d=d||{};f.isObject(b)&&(d=b,b=null);b||(b=this.results?ths.results:[]);var h;0<b.length&&b[0]instanceof c.BaseObject&&(h=c.Object._getClass(b[0].className));return new c.Collection(b,g(d,{model:h,query:this}))};this.fetch=function(b){var d=c.Promise.buildPromise(b);b=this.toRequest(b);b.onSuccess=function(b){m.results=u(b);d.fulfill(m.results)};b.promise=d;b.entity=this;return c.http.send(b)}}})(global);
