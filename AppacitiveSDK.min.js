...
"forEach"in Array.prototype||(Array.prototype.forEach=function(b,f){for(var e=0,c=this.length;e<c;e++)e in this&&b.call(f,this[e],e,this)});"map"in Array.prototype||(Array.prototype.map=function(b,f){for(var e=Array(this.length),c=0,g=this.length;c<g;c++)c in this&&(e[c]=b.call(f,this[c],c,this));return e});"filter"in Array.prototype||(Array.prototype.filter=function(b,f){for(var e=[],c,g=0,h=this.length;g<h;g++)g in this&&b.call(f,c=this[g],g,this)&&e.push(c);return e});
...
(function(){global="undefined"===typeof window?process:window;global.Appacitive||(global.Appacitive={runtime:{isNode:"undefined"!=typeof process,isBrowser:"undefined"!=typeof window}});var b=function(c){if(c&&c.send&&_type.isFunction(c.send))c.onResponse=this.onResponse;else throw Error("No applicable httpTransport class found");var b=[],e={},h=0,g={},f=0;this.addProcessor=function(c){c&&(c.pre=c.pre||function(){},c.post=c.post||function(){},r(c.pre),t(c.post))};var r=function(c){h+=1;e[h]=c;return h},
...
function(){if(0!==b.length){var h=[],f;for(f in g)g.hasOwnProperty(f)&&h.push(g[f]);for(;0<b.length;){var p=b.shift(),q=[];for(f in e)e.hasOwnProperty(f)&&q.push(e[f](p));this.changeRequestForCors(p);c.send(p,h,q)}}};this.onResponse=function(c){console.dir(c)}},f=function(){var c=function(){throw Error("Not Implemented Exception");};this.inOnline=this.send=c;this.onResponse=function(b,e){c()};this.onError=function(b){c()}},e=null,e=global.Appacitive.runtime.isBrowser?XMLHttpRequest:require("xmlhttprequest-with-globalagent").XMLHttpRequest,
...
e.ontimeout=function(){b.reject(e)};e.onprogress=function(){};-1===c.url.indexOf("?")?c.url+="?ua=ie":c.url+="&ua=ie";e.open(c.method,c.url,!0);e.send(c.data);return b},g=function(b){if(!b.url)throw Error("Please specify request url");b.method||(b.method="GET");b.headers||(b.headers=[]);var h={};b.onSuccess&&_type.isFunction(b.onSuccess)||(b.onSuccess=function(){});b.onError&&_type.isFunction(b.onError)||(b.onError=function(){});var g=global.Appacitive.Promise.buildPromise({success:b.onSuccess,error:b.onError}),
f=!0;b.headers.forEach(function(c){"content-type"==c.key.toLowerCase()&&(f=!0,"application/json"==c.value.toLowerCase()||"application/javascript"==c.value.toLowerCase()||"application/json; charset=utf-8"==c.value.toLowerCase()||"application/json; charset=utf-8;"==c.value.toLowerCase())&&(f=!1)});if(f)h=b.data;else if(b.data&&(h=b.data,_type.isObject(b.data)))try{h=JSON.stringify(h)}catch(k){}if(!global.navigator||-1==global.navigator.userAgent.indexOf("MSIE 8")&&-1==global.navigator.userAgent.indexOf("MSIE 9")){var q=
...
this)}else g.reject(this)};q.open(b.method,b.url,!0);for(var r=0;r<b.headers.length;r+=1)q.setRequestHeader(b.headers[r].key,b.headers[r].value);global.Appacitive.runtime.isBrowser||q.setRequestHeader("User-Agent","Appacitive-NodeJSSDK");q.send(h);return g}b.data=h;return new c(b)},h=function(){var c=new f;c.isOnline=function(){return!0};var b=function(b,e,h){new g({method:b.method,url:b.url,headers:b.headers,data:b.data,onSuccess:function(g,f){if(g){try{g=JSON.parse(g)}catch(m){}var u=g;if(e.length!=
h.length)throw Error("Callback length and state length mismatch!");for(var v=0;v<e.length;v+=1)e[v].apply({},[u,h[v]]);if("200"<=g.code&&"300">=g.code||g.status&&"200"<=g.status.code&&"300">=g.status.code)c.onResponse(b,g);else g=g||{},g=g.status||g,g.message=g.message||"Bad Request",g.code=g.code||"400",c.onError(b,{responseText:JSON.stringify(g)})}else c.onError(b,{responseText:JSON.stringify({code:"400",message:"Invalid request"})})},onError:function(e){var g={};g.message=e.responseText||"Bad Request";
g.code=e.status||"400";c.onError(b,{responseText:JSON.stringify(g)})}})};c.send=function(c,e,g){if(!global.Appacitive.Session.initialized)throw Error("Initialize Appacitive SDK");_type.isFunction(c.beforeSend)&&c.beforeSend(c);b(c,e,g)};return c};global.Appacitive.http=new function(){var c=new h,e=new b(c),g=!1;this.pause=function(){g=!0};this.unpause=function(){g=!1};this.addProcessor=function(c){var b=Error('Must provide a processor object with either a "pre" function or a "post" function.');if(!c)throw b;
if(!c.pre&&!c.post)throw b;e.addProcessor(c)};this.send=function(b){b.promise=global.Appacitive.Promise.is(b.promise)?b.promise:new global.Appacitive.Promise.buildPromise({error:b.onError});e.enqueueRequest(b);c.isOnline()&&!1===g&&e.notify();return b.promise};this.flush=function(b){b?e.notify():c.isOnline()&&e.notify()};this.onError=function(c,b){var e;if(b&&b.responseText)try{e=JSON.parse(b.responseText)}catch(g){}e=e||{code:b.status,message:b.responseText,referenceid:b.headers.TransactionId};global.Appacitive.logs.logRequest(c,
...
this.objectServiceUrl,b,c)},getMultiGetUrl:function(b,c,g){return String.format("{0}/{1}/multiGet/{2}?fields={3}",this.objectServiceUrl,b,c,f(g))},getCreateUrl:function(b,c){return String.format("{0}/{1}?fields={2}",this.objectServiceUrl,b,f(c))},getGetUrl:function(b,c,g){return String.format("{0}/{1}/{2}?fields={3}",this.objectServiceUrl,b,c,f(g))},getUpdateUrl:function(b,c,g,h){return h?String.format("{0}/{1}/{2}?fields={3}&revision={4}",this.objectServiceUrl,b,c,f(g),h):String.format("{0}/{1}/{2}?fields={3}",
...
this.connectionServiceUrl,b,c,f(g))},getCreateUrl:function(b,c){return String.format("{0}/{1}?fields={2}",this.connectionServiceUrl,b,f(c))},getUpdateUrl:function(b,c,g,h){return h?String.format("{0}/{1}/{2}?fields={3}&revision={4}",this.connectionServiceUrl,b,c,f(g),h):String.format("{0}/{1}/{2}?fields={3}",this.connectionServiceUrl,b,c,f(g))},getDeleteUrl:function(b,c){return String.format("{0}/{1}/{2}",this.connectionServiceUrl,b,c)},getMultiDeleteUrl:function(b){return String.format("{0}/{1}/bulkdelete",
this.connectionServiceUrl,b)},getSearchByArticleUrl:function(b,c,g,h){var l="",l=String.format("{0}/{1}/find/all?label={2}&objectid={3}",this.connectionServiceUrl,b,g,c);if("undefined"!==typeof h&&0<h.length)for(b=0;b<h.length;b+=1)l=l+"&"+h[b];return l},getConnectedArticles:function(b,c,g){var h="",h=String.format("{0}/{1}/{2}/find",this.connectionServiceUrl,b,c);if(g&&g.length&&0<g.length)for(b=0;b<g.length;b+=1)h=0===b?h+("?"+g[b]):h+("&"+g[b]);return h},getInterconnectsUrl:function(){return String.format("{0}/interconnects",
...
this.fileServiceUrl,b,escape(c))},getDownloadUrl:function(b,c){return String.format("{0}/download/{1}?expires={2}",this.fileServiceUrl,b,c)},getDeleteUrl:function(b){return String.format("{0}/delete/{1}",this.fileServiceUrl,b)}};this.query={params:function(b){var c=[];return""===location.search||-1===location.search.indexOf("?")?c:b?(b=b.toLowerCase(),location.search.split("?")[1].split("&").forEach(function(g,h){var l=h.split("="),f=l[1];2<l.length&&l.forEach(function(b,c){0!==b&&1!==b&&(f=f+"="+
l[b])});l[0].toLowerCase()===b&&(c=[l[0],f])}),c):location.search.split("?")[1].split("=")}}}})(global);
(function(b){var f;f=b.Appacitive.runtime.isNode?process.nextTick:setTimeout;var e=function(){if(!(this instanceof e))return new e;this.calls=[]};e.prototype.isResolved=function(){return 1===this.state?!0:!1};e.prototype.isRejected=function(){return 2===this.state?!0:!1};e.prototype.isFulfilled=function(){return 1===this.state||2===this.state?!0:!1};e.prototype.done=function(){var c,g,h=this.state,l=this.value;if(!h)return this;for(;c=this.calls.shift();)if(g=c[0],"function"===typeof c[h]){try{l=
c[h].apply(g,this.value)}catch(f){b.Appacitive.logs.logException({name:f.name,message:f.message,stack:f.stack});if(0==g.calls.length)throw Error({name:f.name,message:f.message,stack:f.stack});g.reject(f)}l instanceof e||e.is(l)?l.then(function(b){g.fulfill(b)},function(b){g.reject(b)}):1===h?g.fulfill(l):g.reject(l)}else 1===h?g.fulfill(l):g.reject(l)};e.prototype.fulfill=function(){if(this.state)return this;this.state=1;this.value=arguments;this.done();return this};e.prototype.resolve=e.prototype.fulfill;
e.prototype.reject=function(){if(this.state)return this;this.state=2;this.reason=this.value=arguments;this.done();return this};e.prototype.then=function(b,g){var h=this,l=new e;this.calls[this.calls.length]=[l,b,g];this.state&&f(function(){h.done()});return l};e.when=function(b){var g=[],h=[],l,m=0,n=new e;if(!b)return n.fulfill(g),n;var p=function(){m+=1;m==l&&(n.state||(0<h.length?n.reject(h,g):n.fulfill(g)))},k=function(l){var m=b[l];m instanceof e||m&&"function"===typeof m.then?f(function(){m.then(function(b){g[l]=
b;p()},function(b){h[l]=b;p()})}):f(function(){try{g[l]=m.call()}catch(b){h[l]=b}p()})};Array.isArray(b)||(b=[b]);l=b.length;for(var q=0;q<l;q+=1)k(q);return n};e.is=function(b){return b instanceof e?!0:!1};e.buildPromise=function(b){var g=new e;_type.isObject(b)&&g.then(b.success,b.error);return g};b.Appacitive.Promise=e})(global);
(function(b){b.Appacitive.eventManager=new function(){function b(){var c=function(){return Math.floor(65536*Math.random()).toString(16)};return c()+c()+"-"+c()+"-"+c()+"-"+c()+"-"+c()+c()+c()}var e={};this.subscribe=function(c,g){if("string"!=typeof c||"function"!=typeof g)throw Error("Incorrect subscription call");"undefined"==typeof e[c]&&(e[c]=[]);var h=b();e[c].push({callback:g,id:h});return h};this.unsubscribe=function(b){if(!b)return!1;var g=-1,h=null,l;for(l in e)for(var f=0;f<e[l].length;f+=
1)if(e[l][f].id==b){g=f;h=l;break}return-1!=g?(e[h].splice(g,1),!0):!1};this.fire=function(b,g,h){if("string"!=typeof b)throw Error("Incorrect fire call");if("undefined"==typeof h||null===h)h={};h.eventName=b;if("undefined"!=typeof e.all)for(var l=0;l<e.all.length;l+=1)e.all[l].callback(g,h);l=function(b,c,g){setTimeout(function(){b(c,g)},0)};if("undefined"!=typeof e[b])for(var f=0;f<e[b].length;f+=1)l(e[b][f].callback,g,h)};this.clearSubscriptions=function(b){if("string"!=typeof b)throw Error("Event Name must be string in EventManager.clearSubscriptions");
...
(function(b){var f=function(e){var c;if(!e||!_type.isObject(e))throw Error("Please specify request options");this.promise=b.Appacitive.Promise.buildPromise(e.callbacks);var g=this.request=new b.Appacitive.HttpRequest;c=b.Appacitive.storage.urlFactory[e.type];var h=e.op.replace("get","").replace("Url","")+" "+e.type;c=b.Appacitive.config.apiBaseUrl+c[e.op].apply(c,e.args||[]);g.url=c;g.description=h;g.method=e.method||"get";g.data=e.data||{};g.onSuccess=e.onSuccess;g.onError=e.onError;g.promise=this.promise;
e.entity&&(g.entity=e.entity);return this};f.prototype.send=function(){return b.Appacitive.http.send(this.request)};b.Appacitive._Request=f})(global);
(function(b){b.Appacitive.Session=new function(){this.initialized=!1;var f=function(){this.apikey="";this.isnonsliding=!1;this.usagecount=-1;this.windowtime=240},e=null,c=null,g=null,h=!1;this.useApiKey=!0;this.onSessionCreated=function(){};this.recreate=function(c){return b.Appacitive.Session.create(c)};this.create=function(g){if(!this.initialized)throw Error("Intialize Appacitive SDK");var h=new f;h.apikey=c;var l=new b.Appacitive._Request({method:"PUT",type:"application",op:"getSessionCreateUrl",
...
function(b){return new g({field:this.name,fieldType:this.type,value:b,operator:k.isEqualTo})};f.between=function(b,c){return new h({field:this.name,fieldType:this.type,val1:new q(b,"number"),val2:new q(c,"number"),operator:k.between})};f.betweenDate=function(b,c){return new h({field:this.name,fieldType:this.type,val1:new r(b),val2:new r(c),operator:k.between})};f.betweenTime=function(b,c){return new h({field:this.name,fieldType:this.type,val1:new t(b),val2:new t(c),operator:k.between})};f.betweenDateTime=
function(b,c){return new h({field:this.name,fieldType:this.type,val1:new u(b),val2:new u(c),operator:k.between})};f.withinCircle=function(b,c,g){return new l({field:this.name,fieldType:this.type,geoCoord:b,distance:c,unit:g,operator:k.withinCircle})};f.withinPolygon=function(b){return new m({field:this.name,fieldType:this.type,geoCoords:b,operator:k.withinPolygon})}},B=function(b){if(!b||0===b.length)throw Error("Specify field name");this.field=b;v("property",b,this);return this},A=function(b){if(!b||
...
(this.object=g.object);this.returnEdge=!0;void 0===g.returnEdge||(null===g.returnEdge||g.returnEdge||this.prev)||(this.returnEdge=!1);this.label="";var l=this;_type.isString(g.label)&&0<g.label.length&&(this.label="&label="+g.label);this.toRequest=function(){var c=new b.Appacitive.HttpRequest,g=this.toUrl();c.url=g.url;c.description=g.description;c.method="get";return c};this.toUrl=function(){return{url:b.Appacitive.config.apiBaseUrl+"connection/"+this.relation+"/"+this.type+"/"+this.objectId+"/find?"+
this.getQueryString()+this.label+"&returnEdge="+this.returnEdge,description:"GetConnectedObjects for relation "+this.relation+" of type "+this.type+" for object "+this.objectId}};var f=function(c,g){var e=[];c.forEach(function(c){var h=c.__edge;delete c.__edge;var l=b.Appacitive.Object._create(c,!0);h&&(h.__endpointa=g,h.__endpointb={objectid:c.__id,label:h.__label,type:c.__type},delete h.label,l.connection=b.Appacitive.Connection._create(h,!0));e.push(l)});l.object&&(l.object.children[l.relation]=
...
function(g,e){g=g||{};delete g.entity;if(!g.objectAId||!_type.isString(g.objectAId)||0===g.objectAId.length)throw Error("Specify valid objectAId for GetConnectionsBetweenObjectsQuery query");if(!g.objectBId||!_type.isString(g.objectBId)||0===g.objectBId.length)throw Error("Specify objectBId for GetConnectionsBetweenObjectsQuery query");g.type&&delete g.type;g.queryType=e||"GetConnectionsBetweenObjectsQuery";c.call(this,g);this.objectAId=g.objectAId;this.objectBId=g.objectBId;this.label="GetConnectionsBetweenObjectsForRelationQuery"===
this.queryType()&&g.label&&_type.isString(g.label)&&0<g.label.length?"&label="+g.label:"";this.relation=g.relation&&_type.isString(g.relation)&&0<g.relation.length?g.relation+"/":"";this.toRequest=function(){var c=new b.Appacitive.HttpRequest,g=this.toUrl();c.url=g.url;c.description=g.description;c.method="get";return c};this.toUrl=function(){return{url:b.Appacitive.config.apiBaseUrl+"connection/"+this.relation+"find/"+this.objectAId+"/"+this.objectBId+"?"+this.getQueryString()+this.label,description:"FindConnectionBetween for relation "+
...
"connection/interconnects?"+this.getQueryString(),description:"GetInterConnections between objects"}};return this};b.Appacitive.Queries.InterconnectsQuery.prototype=new c;b.Appacitive.Queries.InterconnectsQuery.prototype.constructor=b.Appacitive.Queries.InterconnectsQuery;b.Appacitive.Queries.GraphFilterQuery=function(c,e){if(!c||0===c.length)throw Error("Specify name of filter query");this.name=c;this.data={};this.queryType="GraphFilterQuery";if(e){this.data.placeholders=e;for(var f in this.data.placeholders)this.data.placeholders[f]=
this.data.placeholders[f]}this.toRequest=function(){var c=new b.Appacitive.HttpRequest,e=this.toUrl();c.url=e.url;c.description=e.description;c.method="post";c.data=this.data;return c};this.toUrl=function(){return{url:b.Appacitive.config.apiBaseUrl+"search/"+this.name+"/filter",description:"Filter Query with name "+this.name}};this.fetch=function(c){var e=b.Appacitive.Promise.buildPromise(c);c=this.toRequest();c.onSuccess=function(b){e.fulfill(b.ids?b.ids:[])};c.promise=e;c.entity=this;return b.Appacitive.http.send(c)}};
b.Appacitive.Queries.GraphProjectQuery=function(c,e,f){if(!c||0===c.length)throw Error("Specify name of project query");if(!e||!e.length)throw Error("Specify ids to project");this.name=c;this.data={ids:e};this.queryType="GraphProjectQuery";if(f){this.data.placeholders=f;for(var m in this.data.placeholders)this.data.placeholders[m]=this.data.placeholders[m]}this.toRequest=function(){var c=new b.Appacitive.HttpRequest,e=this.toUrl();c.url=e.url;c.description=e.description;c.method="post";c.data=this.data;
return c};this.toUrl=function(){return{url:b.Appacitive.config.apiBaseUrl+"search/"+this.name+"/project",description:"Project Query with name "+this.name}};var n=function(c){var e,g;for(g in c)if("status"!==g){e=c[g];break}var h=function(c,e,g){var f=[];c.forEach(function(c){var l=c.__children;delete c.__children;var k=c.__edge;delete c.__edge;c=b.Appacitive.Object._create(c,!0);c.children={};for(var m in l)c.children[m]=[],c.children[m]=h(l[m].values,l[m].parent,c.id);k&&(k.__endpointa={objectid:g,
label:e},k.__endpointb={objectid:c.id(),label:k.__label},delete k.__label,c.connection=b.Appacitive.Connection._create(k,!0));f.push(c)});return f};return h(e.values)};this.fetch=function(c){var e=b.Appacitive.Promise.buildPromise(c);c=this.toRequest();c.onSuccess=function(b){e.fulfill(n(b))};c.promise=e;c.entity=this;return b.Appacitive.http.send(c)}}})(global);
(function(b){var f={},e=Array.prototype.forEach,c=Object.prototype.hasOwnProperty,g=Array.prototype.slice,h=0,l=function(){return{keys:Object.keys,uniqueId:function(b){var c=++h+"";return b?b+c:c},has:function(b,e){return c.call(b,e)},each:function(b,c,g){if(null!=b)if(e&&b.forEach===e)b.forEach(c,g);else if(b.length===+b.length)for(var h=0,l=b.length;h<l&&c.call(g,b[h],h,b)!==f;h++);else for(h in b)if(this.has(b,h)&&c.call(g,b[h],h,b)===f)break},once:function(b){var c=!1,e;return function(){if(c)return e;
c=!0;e=b.apply(this,arguments);b=null;return e}}}}(),m;m={on:function(b,c,e){if(!p(this,"on",b,[c,e])||!c)return this;this._events||(this._events={});(this._events[b]||(this._events[b]=[])).push({callback:c,context:e,ctx:e||this});return this},once:function(b,c,e){if(!p(this,"once",b,[c,e])||!c)return this;var g=this,h=l.once(function(){g.off(b,h);c.apply(this,arguments)});h._callback=c;return this.on(b,h,e)},off:function(b,c,e){var g,h,f,k,m,n,x,w;if(!this._events||!p(this,"off",b,[c,e]))return this;
if(!b&&!c&&!e)return this._events={},this;k=b?[b]:l.keys(this._events);m=0;for(n=k.length;m<n;m++)if(b=k[m],f=this._events[b]){this._events[b]=g=[];if(c||e)for(x=0,w=f.length;x<w;x++)h=f[x],(c&&c!==h.callback&&c!==h.callback._callback||e&&e!==h.context)&&g.push(h);g.length||delete this._events[b]}return this},trigger:function(b){if(!this._events)return this;var c=g.call(arguments,1);if(!p(this,"trigger",b,c))return this;var e=this._events[b],h=this._events.all;e&&k(e,c);h&&k(h,arguments);return this},
...
c.__tags.indexOf(e)&&b.push(e)});return 0<b.length?b:null};this.getChangedTags=A;this.getRemovedTags=function(){return _removetags};var z=function(b,c,e){if(!b||!_type.isString(b)||0===b.length||0==b.trim().indexOf("__")||0===b.trim().indexOf("$")||void 0==c||null==c)return this;b=b.toLowerCase();try{var g=function(c){if(!_type.isString(c))if(_type.isNumber(c)||_type.isBoolean(c))c+="";else throw Error("Multivalued property cannot have values of property as an object");if(k[b]&&_type.isArray(k[b])){var g=
k[b].indexOf(c);"additems"==e?k[b].push(c):-1==g&&"adduniqueitems"==e?k[b].push(c):"removeitems"==e&&k[b].removeAll(c)}else"removeitems"!=e&&(k[b]=[c]);h[b]||(h[b]={additems:[],adduniqueitems:[],removeitems:[]});h[b][e].push(c)};_type.isArray(c)?c.forEach(function(b){g(b)}):g(c);w(b)}catch(f){throw Error("Unable to add item to "+b);}return n};this.add=function(b,c){return z.apply(this,[b,c,"additems"])};this.addUnique=function(b,c){return z.apply(this,[b,c,"adduniqueitems"])};this.remove=function(b,
c){return z.apply(this,[b,c,"removeitems"])};var y=function(b){var e=!1,f=JSON.parse(JSON.stringify(c)),m;for(m in k)null==k[m]||void 0==k[m]?(f[m]=null,e=!0):k[m]!=c[m]?"__tags"==m||"__attributes"==m?delete f[m]:_type.isArray(k[m])?h[m]&&!l[m]?(f[m]=h[m],e=!0):k[m].equals(c[m])?delete f[m]:(f[m]=k[m],e=!0):(f[m]=g[m]&&!l[m]?{incrementby:g[m].value}:k[m],e=!0):k[m]==c[m]&&delete f[m];try{t.forEach(function(b){f[b]&&delete f[b]})}catch(n){}m=A();b?(m&&(f.__addtags=m,e=!0),q&&0<q.length&&(f.__removetags=
q,e=!0)):m&&(f.__tags=m,e=!0);(b=B())?(f.__attributes=b,e=!0):delete f.__attributes;for(var p in f)"$"==p[0]&&delete f[p];return e&&!Object.isEmpty(f)?f:!1};this.changed=function(){return this.isNew()?this.toJSON():y()};this.hasChanged=function(){if(this.isNew())return!0;var b=y(!0);return 0===arguments.length?Object.isEmpty(b)?!1:!0:1==arguments.length&&_type.isString(arguments[0])&&0<arguments[0].length&&b&&b[arguments[0]]?!0:!1};this.changedAttributes=function(){if(this.isNew())return this.toJSON();
...
b?b:c};var w=function(b){var c=y();c[b]&&(n.trigger("change:"+b,n,c[b],{}),n.trigger("change",n,{}))};this.set=function(e,f,m){if(!e||!_type.isString(e)||0===e.length||0===e.trim().indexOf("$"))return this;e=e.toLowerCase();try{if(void 0==f||null==f)k[e]=null;else if(_type.isString(f))k[e]=f;else if(_type.isNumber(f)||_type.isBoolean(f))k[e]=f+"";else if(f instanceof Date)k[e]="date"==m?b.Appacitive.Date.toISODate(f):"time"==m?b.Appacitive.Date.toISOTime(f):b.Appacitive.Date.toISOString(f);else if(_type.isObject(f))if(-1!==
u.indexOf(e))k[e]=f;else if(f instanceof b.Appacitive.GeoCoord)k[e]=f.toString();else throw Error("Property cannot have value as an object");else _type.isArray(f)&&(k[e]=[],f.forEach(function(b){if(_type.isString(b))k[e].push(b);else if(_type.isNumber(b)||_type.isBoolean(b))k[e].push(b+"");else{if(f instanceof Date)throw Error("Multivalued property cannot have values of property as date");throw Error("Multivalued property cannot have values of property as an object");}}),"tags"!==e||"__link"!==e)&&
(k[e].push=function(b){var c=this.length;if(_type.isString(b))this[c]=b;else if(_type.isNumber(b)||_type.isBoolean(b))this[c]=b+"";else throw Error("Multivalued property cannot have values of property as an object");w(e);return this});delete g[e];delete h[e];delete l[e];k[e]!==c[e]&&(_type.isArray(k[e])&&!k[e].equals(c[e])||_type.isString(k[e])||_type.isObject(k[e]))&&(l[e]=!0);w(e);return this}catch(n){throw Error("Unable to set "+e);}};this.unset=function(b){if(!b||!_type.isString(b)||0===b.length||
0===b.indexOf("__"))return this;b=b.toLowerCase();return this.set(b,null)};this.has=function(b){return b&&_type.isString(b)&&0!==b.length?k[b]&&!_type.isUndefined(k[b])?!0:!1:!1};this.isNew=function(){return k.__id&&k.__id.length?!1:!0};this.clone=function(){return"object"==this.type?b.Appacitive.Object._create(this.toJSON()):new b.Appacitive.connection._create(this.toJSON())};this.copy=function(b,e){b&&(m(b,k),e&&m(b,c));return this};this.mergeWithPrevious=function(){m(k,c);q=[];g={};h={};l={};return this};
var C=function(){m(c,k);q=[];g={};h={};l={}};this.rollback=function(){k=p={};C();return this};var D=function(b,c,e){if(!b||!_type.isString(b)||0===b.length||0===b.indexOf("__"))return this;b=b.toLowerCase();if(_type.isObject(k[b])||_type.isArray(k[b]))throw Error("Cannot increment/decrement array/object");try{c=!c||isNaN(Number(c))?e:Number(c)*e,k[b]=isNaN(Number(k[b]))?c:Number(k[b])+c,n.isNew()||(g[b]={value:(g[b]?g[b].value:0)+c})}catch(f){throw Error("Cannot perform increment/decrement operation");
}w(b);return n};this.increment=function(b,c){return D(b,c,1)};this.decrement=function(b,c){return D(b,c,-1)};this.save=function(){return k.__id?F.apply(this,arguments):E.apply(this,arguments)};var E=function(e){var g=n.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(g=k.__type.toLowerCase());for(var f in k)"$"==f[0]&&delete k[f];k.__revision&&delete k.__revision;var h=new b.Appacitive._Request({method:"PUT",type:g,op:"getCreateUrl",args:[this.className,r],data:k,
callbacks:e,entity:n,onSuccess:function(e){var f=null;e&&(e.object||e.connection||e.user||e.device)&&(f=e.object||e.connection||e.user||e.device);e&&f?(c=f,k.__id=f.__id,C(),"connection"==n.type&&n.parseConnection(),b.Appacitive.eventManager.fire(n.entityType+"."+g+".created",n,{object:n}),n.created=!0,h.promise.fulfill(n)):(b.Appacitive.eventManager.fire(n.entityType+"."+g+".createFailed",n,{error:e.status}),h.promise.reject(e.status,n))}});return h.send()},F=function(e,f){b.Appacitive.Promise.is(f)||
(f=b.Appacitive.Promise.buildPromise(e));var g=y(!0),h;for(h in g)"$"==h[0]&&delete g[h];if(Object.isEmpty(g))f.fulfill(n);else{var l=n.type;h=[n.className,c.__id?c.__id:k.__id,r];k&&(k.__type&&("user"==k.__type.toLowerCase()||"device"==k.__type.toLowerCase()))&&(l=k.__type.toLowerCase(),h.splice(0,1));var m=new b.Appacitive._Request({method:"POST",type:l,op:"getUpdateUrl",args:h,data:g,callbacks:e,entity:n,onSuccess:function(e){e&&e[l]?(c=e[l],C(),delete n.created,b.Appacitive.eventManager.fire(n.entityType+
...
var f=this.type;!k.__type||"user"!=k.__type.toLowerCase()&&"device"!=k.__type.toLowerCase()||(f=k.__type.toLowerCase());var g=new b.Appacitive._Request({method:"DELETE",type:f,op:"getDeleteUrl",args:[this.className,k.__id,e],callbacks:c,entity:this,onSuccess:function(b){g.promise.fulfill(b);b&&b.status?g.promise.fulfill(b.status):(b=b||{},b.status=b.status||{},b.status=v(b.status),g.promise.reject(b.status,n))}});return g.send()}};b.Appacitive.BaseObject.prototype.toString=function(){return JSON.stringify(this.getObject())};
b.Appacitive.Events.mixin(b.Appacitive.BaseObject.prototype)})(global);
(function(b){var f=function(){return Math.floor(65536*Math.random()).toString(16)};b.Appacitive.GUID=function(){var b=f()+f()+"-"+f()+"-"+f()+"-"+f()+"-"+f()+f()+f(),c="",g,h,l,m,n,p,k=0,b=b.replace(/\r\n/g,"\n");h="";for(l=0;l<b.length;l++)m=b.charCodeAt(l),128>m?h+=String.fromCharCode(m):(127<m&&2048>m?h+=String.fromCharCode(m>>6|192):(h+=String.fromCharCode(m>>12|224),h+=String.fromCharCode(m>>6&63|128)),h+=String.fromCharCode(m&63|128));for(b=h;k<b.length;)g=b.charCodeAt(k++),h=b.charCodeAt(k++),
l=b.charCodeAt(k++),m=g>>2,g=(g&3)<<4|h>>4,n=(h&15)<<2|l>>6,p=l&63,isNaN(h)?n=p=64:isNaN(l)&&(p=64),c=c+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(m)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(g)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(n)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(p);return c.toString()}})(global);
...
function(e,f,g){if(!_type.isString(e))throw Error("Appacitive.Connection.extend's first argument should be the relation-name.");var n=null;f=f||{};f.className=e;n=b.Appacitive._extend(b.Appacitive.Connection,f,g);delete n.extend;n.className=e;return c[e]=n};var c={};b.Appacitive.Connection._create=function(e,f,g){if(this.className)g=this;else if(!g){g=e.__relationtype;if(!_type.isString(g))throw"_getClass requires a string argument.";var n=c[g];n||(n=b.Appacitive.Connection.extend(g),c[g]=n);g=n}return!0==
...
if(!b.templateName)throw Error("template name is mandatory to send an email");var h={to:b.to||[],cc:b.cc||[],bcc:b.bcc||[],subject:b.subject,from:b.from,body:{templatename:b.templateName||"",data:b.data||{},ishtml:!1===b.isHtml?!1:!0}};if(b.useConfig){h.smtp=f.smtp;if(!b.from&&!f.from)throw Error("from is mandatory to send an email. Set it in config or send it in options");h.from=b.from||f.from;h.replyto=b.replyTo||f.replyto}return e(h,g)};this.sendRawEmail=function(b,g){if(!b||!b.to||!b.to.length||
0===b.to.length)throw Error("Atleast one receipient is mandatory to send an email");if(!b.subject||0===b.subject.trim().length)throw Error("Subject is mandatory to send an email");if(!b.from&&f.from)throw Error("from is mandatory to send an email. Set it in config or send it in options on the portal");if(!b.body)throw Error("body is mandatory to send an email");var h={to:b.to||[],cc:b.cc||[],bcc:b.bcc||[],subject:b.subject,from:b.from,body:{content:b.body||"",ishtml:!1===b.isHtml?!1:!0}};if(b.useConfig){h.smtp=
f.smtp;if(!b.from&&!f.from)throw Error("from is mandatory to send an email. Set it in config or send it in options");h.from=b.from||f.from;h.replyto=b.replyTo||f.replyto}return e(h,g)}}})(global);
...
(function(b){b.Appacitive.File=function(f){f=f||{};this.fileId=f.fileId;this.contentType=f.contentType;this.fileData=f.fileData;var e=this,c=function(c,f,g,h){var l=new b.Appacitive.HttpRequest;l.url=c;l.method="GET";l.description=h;l.onSuccess=f;l.promise=g;l.entity=e;b.Appacitive.http.send(l)},g=function(c,f,g,h,l){(new FormData).append("fileToUpload",f);var r=new b.Appacitive.HttpRequest;r.url=c;r.method="PUT";r.log=!1;r.description="Upload file";r.data=f;r.headers.push({key:"content-type",value:g});
r.send().then(h,function(){l.reject(d,e)})};this.save=function(b){return this.fileId&&_type.isString(this.fileId)&&0<this.fileId.length?l(b):h(b)};var h=function(f){if(!e.fileData)throw Error("Please specify filedata");if(!e.contentType)try{e.contentType=e.fileData.type}catch(h){}e.contentType&&_type.isString(e.contentType)&&0!==e.contentType.length||(e.contentType="text/plain");var l=b.Appacitive.Promise.buildPromise(f);f=b.Appacitive.config.apiBaseUrl+b.Appacitive.storage.urlFactory.file.getUploadUrl(e.contentType,
e.fileId?e.fileId:"");c(f,function(b){g(b.url,e.fileData,e.contentType,function(){e.fileId=b.id;e.getDownloadUrl().then(function(b){return l.fulfill(b,e)},function(b){return l.reject(b)})},l)},l," Get upload url for file ");return l},l=function(f){if(!e.fileData)throw Error("Please specify filedata");if(!e.contentType)try{e.contentType=e.fileData.type}catch(h){}e.contentType&&_type.isString(e.contentType)&&0!==e.contentType.length||(e.contentType="text/plain");var l=b.Appacitive.Promise.buildPromise(f);
...
var h=f[1].split("Z")[0].split(":"),l=h[2].split("."),m=Number(h[0]);1<f.length?(c.setUTCHours(Number(m)),c.setUTCMinutes(Number(h[1])),c.setUTCSeconds(Number(l[0])),l[1]&&c.setUTCMilliseconds(Number(l[1].substring(0,3)))):(c.setHours(Number(m)),c.setMinutes(Number(h[1])),c.setSeconds(Number(l[0])),l[1]&&c.setMilliseconds(Number(l[1].substring(0,3))));return c}catch(n){return null}}})(global);
...
(function(b){b.Appacitive.Cookie=new (b.Appacitive.runtime.isBrowser?function(){this.setCookie=function(f,e,c,g){f=b.Appacitive.getAppPrefix(f);var h="";c&&(h=new Date,h.setTime(h.getTime()+6E4*c),h.toGMTString());h=g?"; expires=Thu, 01-Jan-1970 00:00:01 GMT":b.Appacitive.Session.persistUserToken?"; expires="+(new Date("2020-12-31")).toGMTString():"";c="domain="+window.location.hostname;"localhost"==window.location.hostname&&(c="");document.cookie=f+"="+e+h+"; path=/;"+c};this.readCookie=function(f){f=
