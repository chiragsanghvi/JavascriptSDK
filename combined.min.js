...
function(b){e.push(b)};this.notify=function(){if(0!==e.length){var d=[],f;for(f in g)g.hasOwnProperty(f)&&d.push(g[f]);for(;0<e.length;){var j=e.shift(),l=[];for(f in c)c.hasOwnProperty(f)&&l.push(c[f](j));b.send(j,d,l)}}};this.onResponse=function(b){console.dir(b)}},d=function(){var b=function(){throw Error("Not Implemented Exception");};this.inOnline=this.send=b;this.onResponse=function(){b()};this.onError=function(){b()}},g=null,g=global.Appacitive.runtime.isBrowser?XMLHttpRequest:require("xmlhttprequest").XMLHttpRequest,
...
function(){if(4==this.readyState)if(200<=this.status&&300>this.status||304==this.status){var e=this.responseText;try{var c=this.getResponseHeader("content-type")||this.getResponseHeader("Content-Type");if("application/json"==c.toLowerCase()||"application/javascript"==c.toLowerCase())c=e,global.Appacitive.runtime.isBrowser||"{"!=c[0]&&(c=c.substr(1,c.length-1)),e=JSON.parse(c)}catch(d){}b.onSuccess(e,this)}else b.onError({code:this.status,message:this.statusText},this)};f.open(b.method,b.url,!0);for(var l=
0;l<b.headers.length;l+=1)f.setRequestHeader(b.headers[l].key,b.headers[l].value);global.Appacitive.runtime.isBrowser||f.setRequestHeader("User-Agent","Appacitive-NodeJSSDK");f.send(c);return f},b=function(){var b=new d;b.isOnline=function(){return!0};var e=function(e,d,g,f){f||e.headers.push({key:"content-type",value:"application/json"});new c({method:e.method,url:e.url,headers:e.headers,data:e.data,onSuccess:function(c){try{c=JSON.parse(c)}catch(f){}var k=c;if(d.length!=g.length)throw Error("Callback length and state length mismatch!");
for(var l=0;l<d.length;l+=1)d[l].apply({},[k,g[l]]);b.onResponse(c,e)},onError:function(c){b.onError(e,c)}})};b.send=function(b,c,d){"function"==typeof b.beforeSend&&b.beforeSend(b);e(b,c,d)};b.upload=function(b,c,d){"function"==typeof b.beforeSend&&b.beforeSend(b);e(b,c,d,!0)};return b};global.Appacitive.http=new function(){var e=new b,c=new f(e),d=!1;this.pause=function(){d=!0};this.unpause=function(){d=!1};this.addProcessor=function(b){var e=Error('Must provide a processor object with either a "pre" function or a "post" function.');
if(!b)throw e;if(!b.pre&&!b.post)throw e;c.addProcessor(b)};this.send=function(b){c.enqueueRequest(b);e.isOnline()&&!1==d&&c.notify()};this.flush=function(b){b?c.notify():e.isOnline()&&c.notify()};this.onError=function(b,e){if(b.onError)if(b.context)b.onError.apply(b.context,[e]);else b.onError(e)};e.onError=this.onError;this.onResponse=function(b,e){if(e.onSuccess)if(e.context)e.onSuccess.apply(e.context,[b]);else e.onSuccess(b)};e.onResponse=this.onResponse};global.Appacitive.HttpRequest=function(b){b=
...
b=e;break}return-1!=c?(d[b].splice(c,1),!0):!1};this.fire=function(f,c,b){if("string"!=typeof f)throw Error("Incorrect fire call");if("undefined"==typeof b||null===b)b={};b.eventName=f;if("undefined"!=typeof d.all)for(var e=0;e<d.all.length;e+=1)d.all[e].callback(c,b);e=function(b,e,c){setTimeout(function(){b(e,c)},0)};if("undefined"!=typeof d[f])for(var h=0;h<d[f].length;h+=1)e(d[f][h].callback,c,b)};this.clearSubscriptions=function(f){if("string"!=typeof f)throw Error("Event Name must be string in EventManager.clearSubscriptions");
d[f]&&(d[f].length=0);return this};this.clearAndSubscribe=function(d,c){this.clearSubscriptions(d);this.subscribe(d,c)};this.dump=function(){console.dir(d)}}})(global);(function(f){f.Appacitive.config={apiBaseUrl:"https://apis.appacitive.com/"}})(global);
...
this.__defineGetter__("orderBy",function(){return e});this.__defineSetter__("orderBy",function(b){e=b||"__UtcLastUpdatedDate"});this.__defineGetter__("isAscending",function(){return c});this.__defineSetter__("isAscending",function(b){c="undefined"==typeof b?!1:b});this.orderBy=b.orderBy;this.isAscending=b.isAscending};g.prototype.toString=function(){return"orderBy="+this.orderBy+"&isAsc="+this.isAscending};var c=function(b){var e=b||{},c="",f="",m="",j=b.queryType,n=new d(b),l=new g(b),p=b.schema||
b.relation,q=b.schema?"article":"connection";this.__defineGetter__("type",function(){return q});this.__defineGetter__("entityType",function(){return p});this.__defineGetter__("queryType",function(){return j});this.__defineGetter__("pageQuery",function(){return n});this.__defineGetter__("sortQuery",function(){return l});this.__defineGetter__("filter",function(){return c});this.__defineSetter__("filter",function(b){c=b});this.__defineGetter__("freeText",function(){return f});this.__defineSetter__("freeText",
...
if(!b.articleId)throw Error("Specify articleId for connected articles query");b.schema&&delete b.schema;b.queryType="ConnectedArticlesQuery";var e=new c(b);e.articleId=b.articleId;e.relation=b.relation;e.label="";b.label&&("string"==typeof b.label&&0<b.label.length)&&(e.label="&label="+b.label);e.toRequest=function(){var b=new f.Appacitive.HttpRequest;b.url=this.toUrl();b.method="get";return b};e.toUrl=function(){return f.Appacitive.config.apiBaseUrl+"connection/"+this.relation+"/"+this.articleId+
"/find?"+this.getQueryString()+this.label};return e};f.Appacitive.Queries.GetConnectionsQuery=function(b){b=b||{};if(!b.relation)throw Error("Specify relation for GetConnectionsQuery query");if(!b.articleId)throw Error("Specify articleId for GetConnectionsQuery query");if(!b.label||0==b.label.trim().length)throw Error("Specify label for GetConnectionsQuery query");b.schema&&delete b.schema;b.queryType="GetConnectionsQuery";var e=new c(b);e.articleId=b.articleId;e.relation=b.relation;e.label=b.label;
e.toRequest=function(){var b=new f.Appacitive.HttpRequest;b.url=this.toUrl();b.method="get";return b};e.toUrl=function(){return f.Appacitive.config.apiBaseUrl+"connection/"+e.relation+"/find/all?articleid="+this.articleId+"&label="+this.label+e.getQueryString()};return e};f.Appacitive.Queries.GetConnectionsBetweenArticlesQuery=function(b,e){b=b||{};if(!b.articleAId||"string"!=typeof b.articleAId||0==b.articleAId.length)throw Error("Specify valid articleAId for GetConnectionsBetweenArticlesQuery query");
if(!b.articleBId||"string"!=typeof b.articleBId||0==b.articleBId.length)throw Error("Specify articleBId for GetConnectionsBetweenArticlesQuery query");b.schema&&delete b.schema;b.queryType=e||"GetConnectionsBetweenArticlesQuery";var d=new c(b);d.articleAId=b.articleAId;d.articleBId=b.articleBId;d.label="GetConnectionsBetweenArticlesForRelationQuery"==d.queryType&&b.label&&"string"==typeof b.label&&0<b.label.length?"&label="+b.label:"";d.relation=b.relation&&"string"==typeof b.relation&&0<b.relation.length?
b.relation+"/":"";d.toRequest=function(){var b=new f.Appacitive.HttpRequest;b.url=this.toUrl();b.method="get";return b};d.toUrl=function(){return f.Appacitive.config.apiBaseUrl+"connection/"+this.relation+"find/"+this.articleAId+"/"+this.articleBId+"?"+this.getQueryString()+this.label};return d};f.Appacitive.Queries.GetConnectionsBetweenArticlesForRelationQuery=function(b){b=b||{};if(!b.relation)throw Error("Specify relation for GetConnectionsBetweenArticlesForRelationQuery query");return new f.Appacitive.Queries.GetConnectionsBetweenArticlesQuery(b,
...
c[0]&&(e[c]=b[c])}if(0===arguments.length)return e;if(1==arguments.length)return e[arguments[0]];throw Error(".aggregates() called with an incorrect number of arguments. 0, and 1 are supported.");};var j=[];b.__tags||(b.__tags=[]);this.__defineGetter__("tags",function(){return!b.__tags?[]:b.__tags});this.addTags=function(e){e&&("string"==typeof e&&e.length)&&(b.__tags.push(e),b.__tags=Array.distinct(b.__tags),j&&j.length&&(e=j.indexOf(e),-1!=e&&j.splice(e,1)))};this.removeTags=function(e){e&&("string"==
typeof e&&e.length)&&(j.push(e),j=Array.distinct(j),b.__tags&&b.__tags.length&&(e=b.__tags.indexOf(e),-1!=e&&b.__tags.splice(e,1)))};this.get=function(e){if(e)return b[e]};this.set=function(e,c){if(!e||"string"!=typeof e||0==e.length)return c;null==c||"undefined"==c?b[e]=null:"string"==typeof c?b[e]=c:"number"==typeof c?b[e]=c+"":"object"==typeof c&&0<=c.length&&(b[e]=c);return c};this.copy=function(b){b&&m(b)};this.save=function(e,c){b.__id?l.apply(this,arguments):n.apply(this,arguments)};var n=
function(e,d){e=e||function(){};d=d||function(){};var g=this,h=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[this.type].getCreateUrl(b.__schematype||b.__relationtype);if(b.__schematype&&("user"==b.__schematype.toLowerCase()||"device"==b.__schematype.toLowerCase()))h=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[b.__schematype.toLowerCase()].getCreateUrl();var j=new f.Appacitive.HttpRequest;j.url=h;j.method="put";b.__revision&&delete b.__revision;j.data=b;j.onSuccess=
function(f){var h=null;if(f&&(f.article||f.connection||f.user||f.device))h=f.article||f.connection||f.user||f.device;f&&h?(c=h,b.__id=h.__id,m(h),g.connectionCollections&&0<g.connectionCollections.length&&g.connectionCollections.forEach(function(e){e.getQuery().extendOptions({articleId:b.__id})}),"connection"==g.type&&g.parseConnection(),Appacitive.eventManager.fire((g.__schematype||g.__relationtype)+"."+g.type+".created","base",{object:g}),"function"==typeof e&&e(g)):(f=f||{},f.status=f.status||
{},f.status=k(f.status),Appacitive.eventManager.fire((g.__schematype||g.__relationtype)+"."+g.type+".createFailed","base",{error:f.status}),"function"==typeof d&&d(f.status,g))};j.onError=function(b){b=k(b);"function"==typeof d&&d(b,g)};f.Appacitive.http.send(j)},l=function(e,d){e=e||function(){};d=d||function(){};var g=!1,h=JSON.parse(JSON.stringify(c)),n;for(n in b)"undefined"==typeof b[n]||null===b[n]?(h[n]=null,g=!0):b[n]!=c[n]?(h[n]=b[n],g=!0):b[n]==c[n]&&delete h[n];try{h.__revision&&delete h.__revision,
h.__endpointa&&delete h.__endpointa,h.__endpointb&&delete h.__endpointb,h.__createdby&&delete h.__createdby,h.__lastmodifiedby&&delete h.__lastmodifiedby,h.__schematype&&delete h.__schematype,h.__relationtype&&delete h.__relationtype,h.__utcdatecreated&&delete h.__utcdatecreated,h.__utclastupdateddate&&delete h.__utclastupdateddate,h.__tags&&delete h.__tags}catch(l){}b.__tags&&0<b.__tags.length&&(h.__addtags=b.__tags);j&&0<j.length&&(h.__removetags=j);var r=this;if(g){g=new f.Appacitive.HttpRequest;
n=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[this.type].getUpdateUrl(b.__schematype||b.__relationtype,c.__id?c.__id:b.__id);if(b&&b.__schematype&&("user"==b.__schematype.toLowerCase()||"device"==b.__schematype.toLowerCase()))n=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[b.__schematype.toLowerCase()].getUpdateUrl(c.__id);g.url=n;g.method="post";g.data=h;g.onSuccess=function(f){f&&(f.article||f.connection||f.user||f.device)?(c=f.article||f.connection||f.user||
f.device,m(c),Appacitive.eventManager.fire(r.type+"."+b.__id+".updated","base",{object:r}),"function"==typeof e&&e(r)):(f=f||{},f.status=f.status||{},f.status=k(f.status),Appacitive.eventManager.fire(r.type+"."+b.__id+".updateFailed","base",{object:f.status}),"function"==typeof d&&d(f.status,r))};g.onError=function(b){b=b||{};b.message=b.message||"Server error";b.code=b.code||"500";"function"==typeof d&&d(b,r)};f.Appacitive.http.send(g)}else"function"==typeof e&&e(r)};this.fetch=function(e,d){e=e||
function(){};d=d||function(){};if(b.__id){var g=this,h=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[this.type].getGetUrl(b.__schematype||b.__relationtype,b.__id),j=new f.Appacitive.HttpRequest;j.url=h;j.method="get";j.onSuccess=function(b){b&&(b.article||b.connection||b.user||b.device)?(c=b.article||b.connection||b.user||b.device,m(c),g.___collection&&"article"==g.___collection.collectionType&&g.___collection.addToCollection(g),"function"==typeof e&&e(g)):(b=b||{},b.status=b.status||
{},b.status=k(b),"function"==typeof d&&d(b.status,g))};j.onError=function(b){b=k(b);"function"==typeof d&&d(b,g)};f.Appacitive.http.send(j)}else"function"==typeof d&&d({code:"400",message:"Please specify id for get operation"},this)};this.del=function(e,c,d){if(!b.__id&&this.___collection)this.___collection.removeByCId(this.__cid),"function"==typeof e&&e(this);else{e=e||function(){};c=c||function(){};d=d||{};var g=this,h=f.Appacitive.config.apiBaseUrl,h=h+f.Appacitive.storage.urlFactory[this.type].getDeleteUrl(b.__schematype||
b.__relationtype,b.__id);if(b&&b.__schematype&&("user"==b.__schematype.toLowerCase()||"device"==b.__schematype.toLowerCase()))h=f.Appacitive.config.apiBaseUrl,h+=f.Appacitive.storage.urlFactory[b.__schematype.toLowerCase()].getDeleteUrl(b.__id);d.deleteConnections&&!0===d.deleteConnections&&(h=-1==h.indexOf("?")?h+"?deleteconnections=true":h+"&deleteconnections=true");d=new f.Appacitive.HttpRequest;d.url=h;d.method="delete";d.onSuccess=function(d){"200"==d.code?(g.___collection&&g.___collection.removeById(b.__id),
"function"==typeof e&&e(d)):(d=k(d),"function"==typeof c&&c(d,g))};d.onError=function(b){b=k(b);"function"==typeof c&&c(b,g)};f.Appacitive.http.send(d)}}};f.Appacitive.BaseObject.prototype.toString=function(){return JSON.stringify(this.getObject())}})(global);
(function(f){var d=function(){return Math.floor(65536*Math.random()).toString(16)};f.Appacitive.GUID=function(){var f=d()+d()+"-"+d()+"-"+d()+"-"+d()+"-"+d()+d()+d(),c="",b,e,h,k,m,j,n=0,f=f.replace(/\r\n/g,"\n");e="";for(h=0;h<f.length;h++)k=f.charCodeAt(h),128>k?e+=String.fromCharCode(k):(127<k&&2048>k?e+=String.fromCharCode(k>>6|192):(e+=String.fromCharCode(k>>12|224),e+=String.fromCharCode(k>>6&63|128)),e+=String.fromCharCode(k&63|128));for(f=e;n<f.length;)b=f.charCodeAt(n++),e=f.charCodeAt(n++),
h=f.charCodeAt(n++),k=b>>2,b=(b&3)<<4|e>>4,m=(e&15)<<2|h>>6,j=h&63,isNaN(e)?m=j=64:isNaN(h)&&(j=64),c=c+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(k)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(b)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(m)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(j);return c.toString()}})(global);
(function(f){f.Appacitive.ArticleCollection=function(d){var g=null,c=null,b=[],e=d;this.collectionType="article";if(!d||!d.schema)throw Error("Must provide schema while initializing ArticleCollection.");var g=d.schema,h=this,k=function(b){b.type="article";b.schema?g=b.schema:b.schema=g;c=new f.Appacitive.Queries.BasicFilterQuery(b);e=b;h.extendOptions=c.extendOptions};this.setFilter=function(b){e.filter=b;e.type="article";c?c.filter=b:(c=new f.Appacitive.Queries.BasicFilterQuery(e),h.extendOptions=
...
var m=["BasicFilterQuery"];this.__defineSetter__("query",function(e){if(!e||!e.toRequest)throw Error("Invalid  appacitive query passed to articleCollection");if(-1==m.indexOf(e.queryType))throw Error("ArticleCollection only accepts "+m.join(", "));b.length=0;c=e});this.setQuery=function(b){this.query=b};this.setOptions=k;k(d);this.get=function(e){if(e!=parseInt(e,10))return null;e=parseInt(e,10);return"number"!=typeof e||e>=b.length?null:b.slice(e,e+1)[0]};this.addToCollection=function(e){if(!e||
e.get("__schematype")!=g)throw Error("Null article passed or schema type mismatch");var c=null;b.forEach(function(b,d){b.get("__id")==e.get("__id")&&(c=d)});0!=c?b.splice(c,1):b.push(e)};this.getArticleById=function(e,c,d){c=c||function(){};d=d||function(){};var f=b.filter(function(b){return b.get("__id")==e});1==f.length?c(Array.prototype.slice.call(f)[0]):d()};this.getAll=function(){return Array.prototype.slice.call(b)};this.getAllArticles=function(){return Array.prototype.slice.call(b).map(function(b){return b.getArticle()})};
this.removeById=function(e){if(!e)return!1;var c=null;b.forEach(function(b,d){b.getArticle().__id&&b.getArticle().__id==e&&(c=d)});return null!==c?(b.splice(c,1),!0):!1};this.removeByCId=function(e){if(!e)return!1;var c=null;b.forEach(function(b,d){b.__cid&&b.__cid==e&&(c=d)});return null!==c?(b.splice(c,1),!0):!1};this.fetch=function(e,d){e=e||function(){};d=d||function(){};b.length=0;var g=c.toRequest();g.onSuccess=function(c){var g=e,k=c.articles;if(k){if(!k.length||0===k.length)k=[];k.forEach(function(e){e=
new f.Appacitive.Article(e,!0);e.___collection=h;b.push(e)});g(c.paginginfo||{})}else d(c.status)};f.Appacitive.http.send(g)};this.fetchByPageNumber=function(b,e,d){c.getOptions().pageQuery.pageNumber=d;this.fetch(b,e)};this.fetchNextPage=function(b,e){c.getOptions().pageQuery.pageNumber+=1;this.fetch(b,e)};this.fetchPreviousPage=function(b,e){var d=c.getOptions().pageQuery;d.pageNumber-=1;0===d.pageNumber&&(d.pageNumber=1);this.fetch(b,e)};this.createNewArticle=function(e){e=e||{};e.__schematype=
g;e=new f.Appacitive.Article(e);e.___collection=h;e.__cid=parseInt(1E6*Math.random(),10);b.push(e);return e};this.map=function(){return b.map.apply(this,arguments)};this.forEach=function(){return b.forEach.apply(this,arguments)};this.filter=function(){return b.filter.apply(this,arguments)}};f.Appacitive.ArticleCollection.prototype.toString=function(){return JSON.stringify(this.getAllArticles())};f.Appacitive.ArticleCollection.prototype.toJSON=function(){return this.getAllArticles()};f.Appacitive.ArticleCollection.prototype.__defineGetter__("articles",
function(){return this.getAll()})})(global);
(function(f){f.Appacitive.ConnectionCollection=function(d){var g=null,c=null,b=[],e=[],h=d;this.collectionType="connection";if(!d||!d.relation)throw Error("Must provide relation while initializing ConnectionCollection.");var g=d.relation,k=function(b){b.type="connection";b.relation?g=b.relation:b.relation=g;c=new f.Appacitive.Queries.BasicFilterQuery(b);h=b};this.setFilter=function(b){h.filter=b;h.type="connection";c?c.filter=b:(c=new f.Appacitive.Queries.BasicFilterQuery(h),j.extendOptions=c.extendOptions)};
this.setFreeText=function(b){!b&&0==b.trim().length&&(h.freeText="");h.freeText=b;h.type="connection";c?c.freeText=b:(c=new f.Appacitive.Queries.BasicFilterQuery(h),j.extendOptions=c.extendOptions)};this.setFields=function(b){b||(h.fields="");h.fields=b;h.type="connection";c?c.fields=b:(c=new f.Appacitive.Queries.BasicFilterQuery(h),j.extendOptions=c.extendOptions)};this.__defineGetter__("query",function(){return c});this.getQuery=function(){return c};var m=["BasicFilterQuery","ConnectedArticlesQuery",
"GetConnectionsQuery","GetConnectionsBetweenArticlesForRelationQuery"];this.__defineSetter__("query",function(d){if(!d||!d.toRequest)throw Error("Invalid  appacitive query passed to connectionCollection");if(-1==m.indexOf(d.queryType))throw Error("ConnectionCollection only accepts "+m.join(", "));e.length=0;b.length=0;c=d});this.setQuery=function(b){this.query=b};this.reset=function(){g=h=null;e.length=0;b.length=0;c=null};this.setOptions=k;k(d);this.get=function(e){if(e!=parseInt(e,10))return null;
e=parseInt(e,10);return"number"!=typeof e||e>=b.length?null:b.slice(e,e+1)[0]};this.addToCollection=function(e){if(!e||e.get("__relationtype")!=g)throw Error("Null connection passed or relation type mismatch");var c=null;b.forEach(function(b,d){b.get("__id")==e.get("__id")&&(c=d)});null!==c?b.splice(c,1):b.push(e)};this.getConnection=function(e,c,d){c=c||function(){};d=d||function(){};var f=b.filter(function(b){return b.get("__id")==e});1==f.length?c(Array.prototype.slice.call(f)[0]):d()};this.getAll=
function(){return Array.prototype.slice.call(b)};this.getAllConnections=function(){return Array.prototype.slice.call(b).map(function(b){return b.getConnection()})};this.removeById=function(e){if(!e)return!1;var c=null;b.forEach(function(b,d){b.getConnection().__id&&b.getConnection().__id==e&&(c=d)});return null!==c?(b.splice(c,1),!0):!1};this.removeByCId=function(e){if(!e)return!1;var c=null;b.forEach(function(b,d){b.__cid&&b.__cid==e&&(c=d)});return null!==c?(b.splice(c,1),!0):!1};var j=this;this.getConnectedArticle=
function(b){if(!e||0===e.length)return null;var c=e.filter(function(e){return e.get("__id")==b});return 0<c.length?c[0]:null};this.fetch=function(d,g){d=d||function(){};g=g||function(){};b.length=0;var h=c.toRequest();h.onSuccess=function(h){a:{var k=d;h=h||{};var m=h.connections;"GetConnectionsBetweenArticlesForRelationQuery"==c.queryType&&h.connection&&(m=[h.connection]);if(!m)if(h.status&&h.status.code&&"200"==h.status.code)m=[];else{g(h.status);break a}if(!m.length||0===m.length)m=[];m.forEach(function(c){c=
new f.Appacitive.Connection(c,!0);c.___collection=j;if(c.endpointA.article||c.endpointB.article){var d=c.endpointA.article||c.endpointB.article;d.___collection=j;e.push(d)}try{c.___collection.connectedArticle||delete c.connectedArticle}catch(g){}b.push(c)});k(h.paginginfo||{})}};f.Appacitive.http.send(h)};this.fetchByPageNumber=function(b,e,d){c.getOptions().pageQuery.pageNumber=d;this.fetch(b,e)};this.fetchNextPage=function(b,e){c.getOptions().pageQuery.pageNumber+=1;this.fetch(b,e)};this.fetchPreviousPage=
function(b,e){var d=c.getOptions().pageQuery;d.pageNumber-=1;0===d.pageNumber&&(d.pageNumber=1);this.fetch(b,e)};this.createNewConnection=function(e){e=e||{};e.__relationtype=g;e=new f.Appacitive.Connection(e);e.___collection=j;e.__cid=parseInt(1E6*Math.random(),10);b.push(e);return e};this.map=function(){return b.map.apply(this,arguments)};this.forEach=function(e,c){return b.forEach(e,c||this)};this.filter=function(){return b.filter.apply(this,arguments)}}})(global);
...
(function(f){var d=function(b,e,c){var d={label:b.label};b.articleid&&(d.articleid=b.articleid);if(b.article)"function"==typeof b.article.getArticle?b.article.get("__id")?d.articleid=b.article.get("__id"):d.article=b.article.getArticle():"object"==typeof b.article&&b.article.__schematype&&(b.article.__id?d.articleid=b.article.__id:d.article=b.article,b.article=new Appacitive.Article(b.article));else if(!d.articleid&&!d.article)throw Error("Incorrectly configured endpoints provided to setupConnection");
c["endpoint"+e]=b;return d},g=function(b,e,c){c.get("__endpoint"+e.toLowerCase()).article&&"object"==typeof c.get("__endpoint"+e.toLowerCase()).article?(c["endpoint"+e]?c["endpoint"+e]&&c["endpoint"+e].article&&c["endpoint"+e].article.getArticle?c["endpoint"+e].article.copy(c.get("__endpointb").article):c["endpoint"+e].article=new f.Appacitive.Article(c.get("__endpoint"+e.toLowerCase()).article):(c["endpoint"+e]={},c["endpoint"+e].article=new f.Appacitive.Article(c.get("__endpoint"+e.toLowerCase()).article)),
c["endpoint"+e].articleid=c.get("__endpoint"+e.toLowerCase()).articleid,c["endpoint"+e].label=c.get("__endpoint"+e.toLowerCase()).label,c["endpoint"+e].type=c.get("__endpoint"+e.toLowerCase()).type,c["endpoint"+e].article.___collection=c.___collection,delete c.get("__endpoint"+e.toLowerCase()).article):c["endpoint"+e]=c.get("__endpoint"+e.toLowerCase())};f.Appacitive.Connection=function(b,e){if(!b.__relationtype&&!b.relation)throw new error("Cannot set connection without relation");b.relation&&(b.__relationtype=
b.relation,delete b.relation);b.endpoints&&2==b.endpoints.length&&(b.__endpointa=b.endpoints[0],b.__endpointb=b.endpoints[1],delete b.endpoints);var c=new f.Appacitive.BaseObject(b);c.type="connection";c.getConnection=c.getObject;c.setupConnection=function(b,e){if(!b||!b.articleid&&!b.article||!b.label||!e||!e.articleid&&!e.article||!e.label)throw Error("Incorrect endpoints configuration passed.");c.set("__endpointa",d(b,"A",c));c.set("__endpointb",d(e,"B",c))};c.parseConnection=function(){var e=
"A",d="B";b.__endpointa.label==this.get("__endpointb").label&&b.__endpointa.articleid==this.get("__endpointb").articleid&&(e="B",d="A");g(this.get("__endpointa"),e,c);g(this.get("__endpointb"),d,c);c.__defineGetter__("endpoints",function(){var b=[];b.push(this.endpointA);b.push(this.endpointB);return b});return c};e?(c.__defineGetter__("connectedArticle",function(){if(!c.___collection.connectedArticle)throw Error("connectedArticle can be accessed only by using the getConnectedArticles call");var b=
c.___collection.connectedArticle.get("__id");if(!b)return null;var e=c.getConnection().__endpointa.articleid;c.getConnection().__endpointa.articleid==b&&(e=c.getConnection().__endpointb.articleid);return c.___collection.getConnectedArticle(e)}),c.parseConnection()):b.__endpointa&&b.__endpointb&&c.setupConnection(c.get("__endpointa"),c.get("__endpointb"));return c};f.Appacitive.Connection.multiDelete=function(b,e,c,d){if(!b)throw Error("Specify relationName");if(0<e.length){var g=new f.Appacitive.HttpRequest;
g.url=f.Appacitive.config.apiBaseUrl+Appacitive.storage.urlFactory.connection.getMultiDeleteUrl(b);g.method="post";g.data={idlist:e};g.onSuccess=function(b){b&&"200"==b.code?c():(b=b||{},d(b||{message:"Server error",code:400}))};g.onError=function(b){b=b||{};d(b||{message:"Server error",code:400})};f.Appacitive.http.send(g)}else c()};var c=function(b){var e=[];b||(b=[]);b.forEach(function(){e.push(new f.Appacitive.Connection(a))});return e};f.Appacitive.Connection.multiGet=function(b,e,d,g){if(!b)throw Error("Specify relationName");
if("object"==typeof e&&0<e.length){var m=new f.Appacitive.HttpRequest;m.url=f.Appacitive.config.apiBaseUrl+Appacitive.storage.urlFactory.connection.getMultiGetUrl(b,e.join(","));m.method="get";m.onSuccess=function(b){b&&b.status&&"200"==b.status.code?"function"==typeof d&&d(c(b.connections)):(b=b||{},"function"==typeof g&&g(b.status||{message:"Server error",code:400}))};m.onError=function(b){b=b||{};"function"==typeof g&&g(b.status||{message:"Server error",code:400})};f.Appacitive.http.send(m)}else d([])};
f.Appacitive.Connection.getBetweenArticles=function(b,e,d,g){b=(new Appacitive.Queries.GetConnectionsBetweenArticlesQuery({articleAId:b,articleBId:e})).toRequest();b.onSuccess=function(b){b&&b.status&&"200"==b.status.code?"function"==typeof d&&d(c(c(b.connections))):(b=b||{},"function"==typeof g&&g(b.status||{message:"Server error",code:400}))};b.onError=function(b){b=b||{};"function"==typeof g&&g(b.status||{message:"Server error",code:400})};f.Appacitive.http.send(b)};f.Appacitive.Connection.getBetweenArticlesForRelation=
function(b,e,c,d,g){b=(new Appacitive.Queries.GetConnectionsBetweenArticlesForRelationQuery({articleAId:b,articleBId:e,relation:c})).toRequest();b.onSuccess=function(b){b&&b.status&&"200"==b.status.code?"function"==typeof d&&(b=b.connection?new f.Appacitive.Connection(b.connection):null,d(b)):(b=b||{},"function"==typeof g&&g(b.status||{message:"Server error",code:400}))};b.onError=function(b){b=b||{};"function"==typeof g&&g(b.status||{message:"Server error",code:400})};f.Appacitive.http.send(b)}})(global);
(function(f){f.Appacitive.Users=new function(){var d=null;this.__defineGetter__("currentUser",function(){return d});this.setCurrentUser=function(g,c,b){if(!g||"object"!=typeof g)throw Error("Cannot set null object as user");var e=g;g.getArticle||(e=new f.Appacitive.User(g));if(!e.get("__id")||0==e.get("__id").length)throw Error("Specify user __id");f.Appacitive.localStorage.set("Appacitive-User",g);b||(b=60);d=e;c&&f.Appacitive.Session.setUserAuthHeader(c,b);d.logout=function(b){f.Appacitive.Users.logout(b)};
...
if(!c.subject||0==c.subject.trim().length)throw Error("Subject is mandatory to send an email");if(!c.from&&d.from)throw Error("from is mandatory to send an email. Set it in config or send it in options on the portal");if(!c.templateName)throw Error("template name is mandatory to send an email");var f={to:c.to||[],cc:c.cc||[],bcc:c.bcc||[],subject:c.subject,body:{templatename:c.templateName||"",data:c.data||{},ishtml:!1==c.isHtml?!1:!0}};if(c.useConfig){f.smtp=d.smtp;if(!c.from&&!d.from)throw Error("from is mandatory to send an email. Set it in config or send it in options");
...
(function(f){f.Appacitive.File=function(d){d=d||{};this.fileId=d.fileId;this.contentType=d.contentType;this.fileData=d.fileData;var g=function(b,c,d){var g=new f.Appacitive.HttpRequest;g.url=b;g.method="GET";g.onSuccess=c;g.onError=d;f.Appacitive.http.send(g)},c=function(b,c,d,g,m){(new FormData).append("fileToUpload",c);var j=new f.Appacitive.HttpRequest;j.url=b;j.method="PUT";j.data=c;j.headers.push({key:"content-type",value:d});j.onSuccess=g;j.onError=m;j.send(!0)};this.save=function(b,e,d){if(this.fileId&&
"string"==typeof this.fileId&&0<this.fileId.length){var k=this,m=b,j=e;if(!k.fileData)throw Error("Please specify filedata");if(d||"string"==typeof d)k.contentType=d;else{if(!k.contentType||"string"!==typeof d||0==k.contentType.length)k.contentType="text/plain";try{k.contentType=file.type}catch(n){}}b=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory.file.getUpdateUrl(k.fileId,k.contentType);m=m||function(){};j=j||function(){};g(b,function(b){b&&b.status&&"200"==b.status.code?c(b.url,
k.fileData,k.contentType,function(){k.getDownloadUrl(function(b){m(b,k)},j)},j):"function"==typeof j&&j(b.status,k)},j)}else{var l=this,p=b,q=e;if(!l.fileData)throw Error("Please specify filedata");if(d||"string"==typeof d)l.contentType=d;else{if(!l.contentType||"string"!==typeof d||0==l.contentType.length)l.contentType="text/plain";try{l.contentType=file.type}catch(t){}}b=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory.file.getUploadUrl(l.contentType);p=p||function(){};q=q||function(){};
g(b,function(b){b&&b.status&&"200"==b.status.code?c(b.url,l.fileData,l.contentType,function(){l.fileId=b.id;l.getDownloadUrl(function(b){p(b,l)},q)},q):"function"==typeof q&&q(b.status,l)},q)}};this.deleteFile=function(b,c){if(!this.fileId)throw Error("Please specify fileId to delete");b=b||function(){};c=c||function(){};var d=new f.Appacitive.HttpRequest;d.url=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory.file.getDeleteUrl(this.fileId);d.method="DELETE";d.onSuccess=function(d){d&&
"200"==d.code?"function"==typeof b&&b():"function"==typeof c&&"function"==typeof c&&c(d,that)};d.onError=c;f.Appacitive.http.send(d)};this.getDownloadUrl=function(b,c){if(!this.fileId)throw Error("Please specify fileId to download");var d=this;b=b||function(){};c=c||function(){};var g=new f.Appacitive.HttpRequest;g.url=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory.file.getDownloadUrl(this.fileId,556E4);g.method="GET";g.onSuccess=function(f){f&&f.status&&"200"==f.status.code?(d.url=
f.uri,"function"==typeof b&&b(f.uri)):"function"==typeof c&&"function"==typeof c&&c(f.status,d)};g.onError=c;f.Appacitive.http.send(g)}}})(global);
(function(f){f.Appacitive.Date={};f.Appacitive.Date.parseISODate=function(d){try{var f=new Date(d);if(isNaN(f)){if(/^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2})T([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z$/.exec(d)){var c=d.split("T"),b=c[0].split("-"),e=c[1].split("Z")[0].split(":"),h=e[2].split("."),k=Number(e[0]),f=new Date;f.setUTCFullYear(Number(b[0]));f.setUTCMonth(Number(b[1])-1);f.setUTCDate(Number(b[2]));f.setUTCHours(Number(k));f.setUTCMinutes(Number(e[1]));f.setUTCSeconds(Number(h[0]));
h[1]&&f.setUTCMilliseconds(Number(h[1]));return f}return null}return f}catch(m){return null}};f.Appacitive.Date.toISOString=function(d){try{return d=d.toISOString(),d=replace("Z","0000Z")}catch(f){return null}}})(global);
(function(f){f.Appacitive.localStorage=new function(){var d=f.Appacitive.runtime.isBrowser?window.localStorage:{};this.set=function(f,c){c=c||"";if(!f)return!1;if("object"==typeof c)try{c=JSON.stringify(c)}catch(b){}d[f]=c;return!0};this.get=function(f){if(!f)return null;f=d.getItem(f);if(!f)return null;if("{"==f[0])try{f=JSON.parse(f)}catch(c){}return f};this.remove=function(f){if(f)try{delete d[f]}catch(c){}}}})(global);
...
"",-1,!0)}})})(global);"undefined"!=typeof module&&(module.exports=function(f){global.Appacitive.initialize(f);return global.Appacitive});