...
(function(){global="undefined"==typeof window?process:window;global.Appacitive||(global.Appacitive={runtime:{isNode:"undefined"!=typeof process,isBrowser:"undefined"!=typeof window}});var f=function(b){if(!b||!b.send||"function"!=typeof b.send)throw Error("No applicable httpTransport class found");b.onResponse=this.onResponse;var c=[],e={},g=0,d={},f=0;this.addProcessor=function(b){if(b){b.pre=b.pre||function(){};b.post=b.post||function(){};var c=b.pre;g+=1;e[g]=c;b=b.post;f+=1;d[f]=b}};this.enqueueRequest=
function(b){c.push(b)};this.notify=function(){if(0!==c.length){var g=[],f;for(f in d)d.hasOwnProperty(f)&&g.push(d[f]);for(;0<c.length;){var k=c.shift(),p=[];for(f in e)e.hasOwnProperty(f)&&p.push(e[f](k));b.send(k,g,p)}}};this.onResponse=function(b){console.dir(b)}},d=function(){var b=function(){throw Error("Not Implemented Exception");};this.inOnline=this.send=b;this.onResponse=function(){b()};this.onError=function(){b()}},g=null,g=global.Appacitive.runtime.isBrowser?XMLHttpRequest:require("xmlhttprequest").XMLHttpRequest,
c=function(b){if("undefined"!==typeof XDomainRequest)throw Error("Appacitive doesn't support versions of IE6, IE7, IE8, IE9 due to crossdomain calls");if(!b.url)throw Error("Please specify request url");b.method||(b.method="GET");b.headers||(b.headers=[]);var c={};try{b.data&&(c=b.data),c=JSON.stringify(c)}catch(e){}if(!b.onSuccess||"function"!=typeof b.onSuccess)b.onSuccess=function(){};if(!b.onError||"function"!=typeof b.onError)b.onError=function(){};var d=new g;d.onreadystatechange=function(){if(4==
this.readyState)if(200<=this.status&&300>this.status||304==this.status){var c=this.responseText;try{var e=this.getResponseHeader("content-type")||this.getResponseHeader("Content-Type");if("application/json"==e.toLowerCase()||"application/javascript"==e.toLowerCase())c=JSON.parse(c)}catch(d){}b.onSuccess(c,this)}else b.onError({code:this.status,message:this.statusText},this)};d.open(b.method,b.url,!0);for(var f=0;f<b.headers.length;f+=1)d.setRequestHeader(b.headers[f].key,b.headers[f].value);global.Appacitive.runtime.isBrowser||
d.setRequestHeader("User-Agent","Appacitive-NodeJSSDK");d.send(c);return d},b=function(){var b=new d;b.isOnline=function(){return!0};var e=function(e,d,g,f){f||e.headers.push({key:"content-type",value:"application/json"});new c({method:e.method,url:e.url,headers:e.headers,data:e.data,onSuccess:function(c){try{c=JSON.parse(c)}catch(f){}var j=c;if(d.length!=g.length)throw Error("Callback length and state length mismatch!");for(var p=0;p<d.length;p+=1)d[p].apply({},[j,g[p]]);b.onResponse(c,e)},onError:function(c){b.onError(e,
c)}})};b.send=function(b,c,d){"function"==typeof b.beforeSend&&b.beforeSend(b);e(b,c,d)};b.upload=function(b,c,d){"function"==typeof b.beforeSend&&b.beforeSend(b);e(b,c,d,!0)};return b};global.Appacitive.http=new function(){var c=new b,e=new f(c),d=!1;this.pause=function(){d=!0};this.unpause=function(){d=!1};this.addProcessor=function(b){var c=Error('Must provide a processor object with either a "pre" function or a "post" function.');if(!b)throw c;if(!b.pre&&!b.post)throw c;e.addProcessor(b)};this.send=
function(b){e.enqueueRequest(b);c.isOnline()&&!1==d&&e.notify()};this.flush=function(b){b?e.notify():c.isOnline()&&e.notify()};this.onError=function(b,c){if(b.onError)if(b.context)b.onError.apply(b.context,[c]);else b.onError(c)};c.onError=this.onError;this.onResponse=function(b,c){if(c.onSuccess)if(c.context)c.onSuccess.apply(c.context,[b]);else c.onSuccess(b)};c.onResponse=this.onResponse};global.Appacitive.HttpRequest=function(b){b=b||{};this.url=b.url||"";this.data=b.data||{};this.headers=b.headers||
[];this.method=b.method||"GET";this.onSuccess=b.onSuccess||function(){};this.onSuccess=b.onError||function(){};this.send=function(){return new __XMLHttp(this)}};var e=global;e.Appacitive&&e.Appacitive.http&&(e.Appacitive.http.addProcessor({pre:function(b){return b},post:function(b,c){try{e.Appacitive.session.isSessionValid(b)?b&&(b.status&&b.status.code&&"8036"==b.status.code||b.code&&"8036"==b.code)?e.Appacitive.Users.logout(function(){},!0):e.Appacitive.session.incrementExpiry():(null!=e.Appacitive.session.get()&&
e.Appacitive.session.resetSession(),e.Appacitive.http.send(c))}catch(d){}}}),e.Appacitive.http.addProcessor({pre:function(){return(new Date).getTime()},post:function(b,c){var e=(new Date).getTime()-c;b._timeTakenInMilliseconds=e}}))})();
...
this.pushServiceUrl)},getGetNotificationUrl:function(d){return String.format("{0}/notification/{1}",this.pushServiceUrl,d)},getGetAllNotificationsUrl:function(d){return String.format("{0}/getAll?psize={1}&pnum={2}",this.pushServiceUrl,d.psize,d.pnum)}};this.file={fileServiceUrl:"file",getUploadUrl:function(){return String.format("{0}/uploadurl?contenttype={1}&expires=20",this.fileServiceUrl,escape(contenttype))},getUploadUrl:function(d,g){return String.format("{0}/updateurl/{1}?contenttype={2}&expires=20",
this.fileServiceUrl,g,escape(contenttype))},getDownloadUrl:function(d,g){return String.format("{0}/download/{1}?expires={2}",this.fileServiceUrl,d,g)},getDeleteUrl:function(){return String.format("{0}/delete/{1}",this.fileServiceUrl,fileId)}};this.query={params:function(d){var g=[];return""==location.search||-1==location.search.indexOf("?")?g:d?(d=d.toLowerCase(),location.search.split("?")[1].split("&").forEach(function(c,b){var e=b.split("="),f=e[1];2<e.length&&e.forEach(function(b){0==b||1==b||
(f=f+"="+e[b])});e[0].toLowerCase()==d&&(g=[e[0],f])}),g):location.search.split("?")[1].split("=")}}}})(global);
...
(function(f){f.Appacitive.Queries={};var d=function(b){b=b||{};var c=1,d=200;this.__defineGetter__("pageNumber",function(){return c});this.__defineSetter__("pageNumber",function(b){c=b||1});this.__defineGetter__("pageSize",function(){return d});this.__defineSetter__("pageSize",function(b){d=b||200});this.pageNumber=b.pageNumber;this.pageSize=b.pageSize};d.prototype.toString=function(){return"psize="+this.pageSize+"&pnum="+this.pageNumber};var g=function(b){b=b||{};var c="__UtcLastUpdatedDate",d=!1;
this.__defineGetter__("orderBy",function(){return c});this.__defineSetter__("orderBy",function(b){c=b||"__UtcLastUpdatedDate"});this.__defineGetter__("isAscending",function(){return d});this.__defineSetter__("isAscending",function(b){d="undefined"==typeof b?!1:b});this.orderBy=b.orderBy;this.isAscending=b.isAscending};g.prototype.toString=function(){return"orderBy="+this.orderBy+"&isAsc="+this.isAscending};var c=function(b){var c=b||{},f="",j="",m="",k=b.queryType,l=new d(b),n=new g(b),r=b.schema||
b.relation,t=b.schema?"article":"connection";this.__defineGetter__("type",function(){return t});this.__defineGetter__("entityType",function(){return r});this.__defineGetter__("queryType",function(){return k});this.__defineGetter__("pageQuery",function(){return l});this.__defineGetter__("sortQuery",function(){return n});this.__defineGetter__("filter",function(){return f});this.__defineSetter__("filter",function(b){f=b});this.__defineGetter__("freeText",function(){return j});this.__defineSetter__("freeText",
function(b){"string"==typeof b?j=b:"object"==typeof b&&b.length&&(j=b.join(" "))});this.__defineGetter__("fields",function(){return m});this.__defineSetter__("fields",function(b){"string"==typeof b?m=b:"object"==typeof b&&b.length&&(m=b.join(","))});this.filter=c.filter||"";this.freeText=c.freeText||"";this.fields=c.fields||"";this.setFilter=function(b){this.filter=b};this.setFreeText=function(b){this.freeText=b};this.setFields=function(b){this.fields=b};this.extendOptions=function(b){for(var f in b)c[f]=
b[f];l=new d(c);n=new g(c)};this.getQueryString=function(){var b=this.pageQuery.toString()+"&"+this.sortQuery.toString();this.filter&&0<this.filter.trim().length&&(b+="&query="+this.filter);this.freeText&&0<this.freeText.trim().length&&(b+="&freetext="+this.freeText+"&language=en");this.fields&&0<this.fields.trim().length&&(b+="&fields="+this.fields);return b};this.getOptions=function(){var b={},c;for(c in this)this.hasOwnProperty(c)&&"function"!=typeof this[c]&&(b[c]=this[c]);return b}};f.Appacitive.Queries.BasicFilterQuery=
...
"&label="+this.label+e.getQueryString()};return e};f.Appacitive.Queries.GraphQuery=function(b){b=b||{};if(!b.graphQuery)throw Error("graphQuery object is mandatory");this.toRequest=function(){var c=new f.Appacitive.HttpRequest;c.url=f.Appacitive.config.apiBaseUrl;c.url+=f.Appacitive.storage.urlFactory.article.getProjectionQueryUrl();c.method="post";c.data=b.graphQuery;return c}}})(global);
...
d.__relationtype.toLowerCase(),this.entityType=d.__relationtype,this.type="connection");var j=function(b){b=b||{};b.message=b.message||"Server error";b.code=b.code||"500"},m=function(c){for(var e in c)if("string"==typeof c[e])b[e]=c[e];else if("object"==typeof c[e]){b[e]=0<=c[e].length?[]:{};for(var d in c[e])b[e][d]=c[e][d]}else b[e]=c[e]};this.getObject=function(){return b};this.toJSON=function(){return b};this.attributes=function(){if(0===arguments.length)return b.__attributes||(b.__attributes=
{}),b.__attributes;if(1==arguments.length)return b.__attributes||(b.__attributes={}),b.__attributes[arguments[0]];if(2==arguments.length){if("string"!==typeof arguments[1])throw Error("only string values can be stored in attributes.");b.__attributes||(b.__attributes={});b.__attributes[arguments[0]]=arguments[1]}else throw Error(".attributes() called with an incorrect number of arguments. 0, 1, 2 are supported.");};this.aggregates=function(){var c={},e;for(e in b){if(!b.hasOwnProperty(e))return;"$"==
e[0]&&(c[e]=b[e])}if(0===arguments.length)return c;if(1==arguments.length)return c[arguments[0]];throw Error(".aggregates() called with an incorrect number of arguments. 0, and 1 are supported.");};var k=[];this.__defineGetter__("tags",function(){return!b.__tags?[]:b.__tags});this.addTags=function(c){c&&("string"==typeof c&&c.length)&&(b.__tags.push(c),b.__tags=Array.distinct(b.__tags),k&&k.length&&(c=k.indexOf(c),-1!=c&&k.splice(c,1)))};this.removeTags=function(c){c&&("string"==typeof c&&c.length)&&
(k.push(c),k=Array.distinct(k),b.__tags&&b.__tags.length&&(c=b.__tags.indexOf(c),-1!=c&&b.__tags.splice(c,1)))};this.get=function(c){if(c)return b[c]};this.set=function(c,e){if(!c||"string"!=typeof c||0==c.length)return e;null==e||"undefined"==e?b[c]=null:"string"==typeof e?b[c]=e:"number"==typeof e?b[c]=e+"":"object"==typeof e&&0<=e.length&&(b[c]=e);return e};this.copy=function(b){b&&m(b)};this.save=function(c,e){b.__id?n.apply(this,arguments):l.apply(this,arguments)};var l=function(e,d){e=e||function(){};
...
(f.article||f.connection||f.user||f.device))h=f.article||f.connection||f.user||f.device;f&&h?(c=h,b.__id=h.__id,m(h),g.connectionCollections&&0<g.connectionCollections.length&&g.connectionCollections.forEach(function(c){c.getQuery().extendOptions({articleId:b.__id})}),"connection"==g.type&&g.parseConnection(),Appacitive.eventManager.fire((g.__schematype||g.__relationtype)+"."+g.type+".created","base",{object:g}),"function"==typeof e&&e(g)):(f=f||{},f.status=f.status||{},f.status=j(f.status),Appacitive.eventManager.fire((g.__schematype||
g.__relationtype)+"."+g.type+".createFailed","base",{error:f.status}),"function"==typeof d&&d(f.status,g))};k.onError=function(b){b=j(b);"function"==typeof d&&d(b,g)};f.Appacitive.http.send(k)},n=function(e,d){e=e||function(){};d=d||function(){};var g=!1,h=JSON.parse(JSON.stringify(c)),l;for(l in b)"undefined"==typeof b[l]||null===b[l]?(h[l]=null,g=!0):b[l]!=c[l]?(h[l]=b[l],g=!0):b[l]==c[l]&&delete h[l];try{h.__revision&&delete h.__revision,h.__endpointa&&delete h.__endpointa,h.__endpointb&&delete h.__endpointb,
h.__createdby&&delete h.__createdby,h.__lastmodifiedby&&delete h.__lastmodifiedby,h.__schematype&&delete h.__schematype,h.__relationtype&&delete h.__relationtype,h.__utcdatecreated&&delete h.__utcdatecreated,h.__utclastupdateddate&&delete h.__utclastupdateddate,h.__tags&&delete h.__tags}catch(n){}b.__tags&&0<b.__tags.length&&(h.__addtags=b.__tags);k&&0<k.length&&(h.__removetags=k);var q=this;if(g){g=new f.Appacitive.HttpRequest;l=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[this.type].getUpdateUrl(b.__schematype||
b.__relationtype,c.__id?c.__id:b.__id);if(b&&b.__schematype&&("user"==b.__schematype.toLowerCase()||"device"==b.__schematype.toLowerCase()))l=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[b.__schematype.toLowerCase()].getUpdateUrl(c.__id);g.url=l;g.method="post";g.data=h;g.onSuccess=function(g){g&&(g.article||g.connection||g.user||g.device)?(c=g.article||g.connection||g.user||g.device,m(c),Appacitive.eventManager.fire(q.type+"."+b.__id+".updated","base",{object:q}),"function"==typeof e&&
e(q)):(g=g||{},g.status=g.status||{},g.status=j(g.status),Appacitive.eventManager.fire(q.type+"."+b.__id+".updateFailed","base",{object:g.status}),"function"==typeof d&&d(g.status,q))};g.onError=function(b){b=b||{};b.message=b.message||"Server error";b.code=b.code||"500";"function"==typeof d&&d(b,q)};f.Appacitive.http.send(g)}else"function"==typeof e&&e(q)};this.fetch=function(e,d){e=e||function(){};d=d||function(){};if(b.__id){var g=this,h=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory[this.type].getGetUrl(b.__schematype||
...
typeof d&&d({code:"400",message:"Please specify id for get operation"},this)};this.del=function(c,e,d){if(!b.__id&&this.___collection)this.___collection.removeByCId(this.__cid),"function"==typeof c&&c(this);else{c=c||function(){};e=e||function(){};d=d||{};var g=this,h=f.Appacitive.config.apiBaseUrl,h=h+f.Appacitive.storage.urlFactory[this.type].getDeleteUrl(b.__schematype||b.__relationtype,b.__id);if(b&&b.__schematype&&("user"==b.__schematype.toLowerCase()||"device"==b.__schematype.toLowerCase()))h=
f.Appacitive.config.apiBaseUrl,h+=f.Appacitive.storage.urlFactory[b.__schematype.toLowerCase()].getDeleteUrl(b.__id);d.deleteConnections&&!0===d.deleteConnections&&(h=-1==h.indexOf("?")?h+"?deleteconnections=true":h+"&deleteconnections=true");d=new f.Appacitive.HttpRequest;d.url=h;d.method="delete";d.onSuccess=function(d){"200"==d.code?(g.___collection&&g.___collection.removeById(b.__id),"function"==typeof c&&c(d)):(d=j(d),"function"==typeof e&&e(d,g))};d.onError=function(b){b=j(b);"function"==typeof e&&
e(b,g)};f.Appacitive.http.send(d)}}};f.Appacitive.BaseObject.prototype.toString=function(){return JSON.stringify(this.getObject())}})(global);
(function(f){var d=function(){return Math.floor(65536*Math.random()).toString(16)};f.Appacitive.GUID=function(){var g=d()+d()+"-"+d()+"-"+d()+"-"+d()+"-"+d()+d()+d(),c="",b,e,f,j,m,k,l=0,g=g.replace(/\r\n/g,"\n");e="";for(f=0;f<g.length;f++)j=g.charCodeAt(f),128>j?e+=String.fromCharCode(j):(127<j&&2048>j?e+=String.fromCharCode(j>>6|192):(e+=String.fromCharCode(j>>12|224),e+=String.fromCharCode(j>>6&63|128)),e+=String.fromCharCode(j&63|128));for(g=e;l<g.length;)b=g.charCodeAt(l++),e=g.charCodeAt(l++),
f=g.charCodeAt(l++),j=b>>2,b=(b&3)<<4|e>>4,m=(e&15)<<2|f>>6,k=f&63,isNaN(e)?m=k=64:isNaN(f)&&(k=64),c=c+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(j)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(b)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(m)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(k);return c.toString()}})(global);
...
this.__defineSetter__("query",function(e){if(!e||!e.toRequest)throw Error("Invalid  appacitive query passed to articleCollection");if("BasicFilterQuery"!==e.type)throw Error("ArticleCollection only accepts BasicFilterQuery");b.length=0;c=e});this.setQuery=function(e){if(!e||!e.toRequest)throw Error("Invalid  appacitive query passed to articleCollection");b.length=0;c=e};this.setOptions=j;j(d);this.get=function(c){if(c!=parseInt(c,10))return null;c=parseInt(c,10);return"number"!=typeof c||c>=b.length?
null:b.slice(c,c+1)[0]};this.addToCollection=function(c){if(!c||c.get("__schematype")!=g)throw Error("Null article passed or schema type mismatch");var e=null;b.forEach(function(b,d){b.get("__id")==c.get("__id")&&(e=d)});0!=e?b.splice(e,1):b.push(c)};this.getArticleById=function(c,e,d){e=e||function(){};d=d||function(){};var g=b.filter(function(b){return b.get("__id")==c});1==g.length?e(Array.prototype.slice.call(g)[0]):d()};this.getAll=function(){return Array.prototype.slice.call(b)};this.getAllArticles=
function(){return Array.prototype.slice.call(b).map(function(b){return b.getArticle()})};this.removeById=function(c){if(!c)return!1;var e=null;b.forEach(function(b,d){b.getArticle().__id&&b.getArticle().__id==c&&(e=d)});return null!==e?(b.splice(e,1),!0):!1};this.removeByCId=function(c){if(!c)return!1;var e=null;b.forEach(function(b,d){b.__cid&&b.__cid==c&&(e=d)});return null!==e?(b.splice(e,1),!0):!1};this.fetch=function(e,d){e=e||function(){};d=d||function(){};b.length=0;var g=c.toRequest();g.onSuccess=
function(c){var g=e,j=c.articles;if(j){if(!j.length||0===j.length)j=[];j.forEach(function(c){c=new f.Appacitive.Article(c,!0);c.___collection=h;b.push(c)});g(c.paginginfo||{})}else d(c.status)};f.Appacitive.http.send(g)};this.fetchByPageNumber=function(b,e,d){c.getOptions().pageQuery.pageNumber=d;this.fetch(b,e)};this.fetchNextPage=function(b,e){c.getOptions().pageQuery.pageNumber+=1;this.fetch(b,e)};this.fetchPreviousPage=function(b,e){var d=c.getOptions().pageQuery;d.pageNumber-=1;0===d.pageNumber&&
(d.pageNumber=1);this.fetch(b,e)};this.createNewArticle=function(c){c=c||{};c.__schematype=g;c=new f.Appacitive.Article(c);c.___collection=h;c.__cid=parseInt(1E6*Math.random(),10);b.push(c);return c};this.map=function(){return b.map.apply(this,arguments)};this.forEach=function(){return b.forEach.apply(this,arguments)};this.filter=function(){return b.filter.apply(this,arguments)}};f.Appacitive.ArticleCollection.prototype.toString=function(){return JSON.stringify(this.getAllArticles())};f.Appacitive.ArticleCollection.prototype.toJSON=
...
!d.toRequest)throw Error("Invalid  appacitive query passed to connectionCollection");e.length=0;b.length=0;c=d});this.setQuery=function(d){if(!d||!d.toRequest)throw Error("Invalid  appacitive query passed to connectionCollection");e.length=0;b.length=0;c=d};this.reset=function(){g=h=null;e.length=0;b.length=0;c=null};this.setOptions=j;j(d);this.get=function(c){if(c!=parseInt(c,10))return null;c=parseInt(c,10);return"number"!=typeof c||c>=b.length?null:b.slice(c,c+1)[0]};this.addToCollection=function(c){if(!c||
c.get("__relationtype")!=g)throw Error("Null connection passed or relation type mismatch");var e=null;b.forEach(function(b,d){b.get("__id")==c.get("__id")&&(e=d)});null!==e?b.splice(e,1):b.push(c)};this.getConnection=function(c,e,d){e=e||function(){};d=d||function(){};var g=b.filter(function(b){return b.get("__id")==c});1==g.length?e(Array.prototype.slice.call(g)[0]):d()};this.getAll=function(){return Array.prototype.slice.call(b)};this.getAllConnections=function(){return Array.prototype.slice.call(b).map(function(b){return b.getConnection()})};
this.removeById=function(c){if(!c)return!1;var e=null;b.forEach(function(b,d){b.getConnection().__id&&b.getConnection().__id==c&&(e=d)});return null!==e?(b.splice(e,1),!0):!1};this.removeByCId=function(c){if(!c)return!1;var e=null;b.forEach(function(b,d){b.__cid&&b.__cid==c&&(e=d)});return null!==e?(b.splice(e,1),!0):!1};var m=this;this.getConnectedArticle=function(b){if(!e||0===e.length)return null;var c=e.filter(function(c){return c.get("__id")==b});return 0<c.length?c[0]:null};this.fetch=function(d,
...
h(c.paginginfo||{})}};f.Appacitive.http.send(h)};this.fetchByPageNumber=function(b,e,d){c.getOptions().pageQuery.pageNumber=d;this.fetch(b,e)};this.fetchNextPage=function(b,e){c.getOptions().pageQuery.pageNumber+=1;this.fetch(b,e)};this.fetchPreviousPage=function(b,e){var d=c.getOptions().pageQuery;d.pageNumber-=1;0===d.pageNumber&&(d.pageNumber=1);this.fetch(b,e)};this.createNewConnection=function(c){c=c||{};c.__relationtype=g;c=new f.Appacitive.Connection(c);c.___collection=m;c.__cid=parseInt(1E6*
Math.random(),10);b.push(c);return c};this.map=function(){return b.map.apply(this,arguments)};this.forEach=function(c,e){return b.forEach(c,e||this)};this.filter=function(){return b.filter.apply(this,arguments)}}})(global);
...
d.updatePassword=function(b,c,e,d){var g=this.get("__id"),n=e,r=d;if(!g||"string"!==typeof g||0==g.length)throw Error("Please specify valid userid");if(!b||"string"!==typeof b||0==b.length)throw Error("Please specify valid oldPassword");if(!c||"string"!==typeof c||0==c.length)throw Error("Please specify valid newPassword");n=n||function(){};r=r||function(){};b==c?"function"==typeof n&&n():(b={oldpassword:b,newpassword:c},c=new f.Appacitive.HttpRequest,c.url=f.Appacitive.config.apiBaseUrl+f.Appacitive.storage.urlFactory.user.getUpdatePasswordUrl(g),
c.method="post",c.data=b,c.onSuccess=function(b){b&&"200"==b.code&&("function"==typeof n?n():r(b))},c.onError=r,f.Appacitive.http.send(c))};Appacitive.eventManager.clearAndSubscribe("user."+e.get("__id")+".updated",function(b,c){f.Appacitive.localStorage.set("Appacitive-User",c.object.getArticle())})};f.Appacitive.User=function(d){d.__schematype="user";return new f.Appacitive.Article(d,!0)};this.deleteUser=function(d,c,b){if(!d)throw Error("Specify userid for user delete");(new Appacitive.Article({__schematype:"user",
...
c()})}catch(d){b(d.message)}}}:new d})(global);
