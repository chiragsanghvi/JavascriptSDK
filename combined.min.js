...
!c.article||!c.label)throw Error("Incorrect endpoints configuration passed.");a.set("__endpointa",d(b,"A",a));a.set("__endpointb",d(c,"B",a))};a.parseConnection=function(){var c="A",d="B";b.__endpointa.articleid==this.get("__endpointa").articleid&&(c="B",d="A");"object"==typeof this.get("__endpointa").article?(this.endpointa?this["endpoint"+c].copy(this.get("__endpointb").getArticle()):this["endpoint"+c]=new e.Appacitive.Article(this.get("__endpointa").article),this["endpoint"+c].___collection=this.___collection):
this["endpoint"+c]=this.get("__endpointa");"object"==typeof this.get("__endpointb").article?(this.endpointb?this["endpoint"+d].copy(this.get("__endpointb").getArticle()):this["endpoint"+d]=new e.Appacitive.Article(this.get("__endpointb").article),this["endpoint"+d].___collection=this.___collection):this["endpoint"+d]=this.get("__endpointb");a.__defineGetter__("endpoints",function(){var a=[];a.push(this.endpointA);a.push(this.endpointB);return a});return a};c?(a.__defineGetter__("connectedArticle",
function(){if(!a.___collection.connectedArticle)throw Error("connectedArticle can be accessed only by using the getConnectedArticles call");var b=a.___collection.connectedArticle.get("__id");if(!b)return null;var c=a.getConnection().__endpointa.articleid;a.getConnection().__endpointa.articleid==b&&(c=a.getConnection().__endpointb.articleid);return a.___collection.getConnectedArticle(c)}),a.parseConnection()):a.setupConnection(a.get("__endpointa"),a.get("__endpointb"));return a};e.Appacitive.Connection.multiDelete=
function(b,c,a,d){if(!b)throw Error("Specify relationName");if(0<c.length){var f=new e.Appacitive.HttpRequest;f.url=e.Appacitive.config.apiBaseUrl+Appacitive.storage.urlFactory.connection.getMultideleteUrl(b);f.method="post";f.data={idlist:c};f.onSuccess=function(b){b&&"200"==b.code?a():(b=b||{},d(b||{message:"Server error",code:400}))};f.onError=function(a){a=a||{};d(a||{message:"Server error",code:400})};e.Appacitive.http.send(f)}else a()}})(global);
