...
(function(f){f.Appacitive.parseISODate=function(d){try{var f=new Date(d);if(isNaN(f)){if(/^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2})T([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z$/.exec(d)){var c=d.split("T"),b=c[0].split("-"),e=c[1].split("Z")[0].split(":"),h=e[2].split("."),k=Number(e[0]),f=new Date;f.setUTCFullYear(Number(b[0]));f.setUTCMonth(Number(b[1])-1);f.setUTCDate(Number(b[2]));f.setUTCHours(Number(k));f.setUTCMinutes(Number(e[1]));f.setUTCSeconds(Number(h[0]));h[1]&&f.setUTCMilliseconds(Number(h[1]));
