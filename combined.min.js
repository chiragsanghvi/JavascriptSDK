...
typeof c.get("__endpoint"+a.toLowerCase()).article?(c["endpoint"+a]?c["endpoint"+a]&&c["endpoint"+a].article&&c["endpoint"+a].article.getArticle?c["endpoint"+a].article.copy(c.get("__endpointb").article):c["endpoint"+a].article=new e.Appacitive.Article(c.get("__endpointb").article):(c["endpoint"+a]={},c["endpoint"+a].article=new e.Appacitive.Article(c.get("__endpointb").article)),c["endpoint"+a].articleid=c.get("__endpoint"+a.toLowerCase()).articleid,c["endpoint"+a].label=c.get("__endpoint"+a.toLowerCase()).label,
c["endpoint"+a].type=c.get("__endpoint"+a.toLowerCase()).type,c["endpoint"+a].article.___collection=c.___collection):c["endpoint"+a]=c.get("__endpoint"+a.toLowerCase())};e.Appacitive.Connection=function(b,a){if(!b.__relationtype&&!b.relation)throw new error("Cannot set connection without schema");b.schema&&(b.__schematype=b.schema,delete b.schema);b.endpoints&&2==b.endpoints.length&&(b.__endpointa=b.endpoints[0],b.__endpointb=b.endpoints[1],delete b.endpoints);if(!b.__endpointa||!b.__endpointb)throw Error("Provide both endpoints.");
var f=new e.Appacitive.BaseObject(b);f.type="connection";f.getConnection=f.getObject;f.setupConnection=function(a,b){if(!a||!a.articleid&&!a.article||!a.label||!b||!b.articleid&&!b.article||!b.label)throw Error("Incorrect endpoints configuration passed.");f.set("__endpointa",d(a,"A",f));f.set("__endpointb",d(b,"B",f))};f.parseConnection=function(){b.__endpointa.label==this.get("__endpointb").label&&this.get("__endpointb");c(this.get("__endpointa"),"A",f);c(this.get("__endpointb"),"B",f);f.__defineGetter__("endpoints",
function(){var a=[];a.push(this.endpointA);a.push(this.endpointB);return a});return f};a?(f.__defineGetter__("connectedArticle",function(){if(!f.___collection.connectedArticle)throw Error("connectedArticle can be accessed only by using the getConnectedArticles call");var a=f.___collection.connectedArticle.get("__id");if(!a)return null;var b=f.getConnection().__endpointa.articleid;f.getConnection().__endpointa.articleid==a&&(b=f.getConnection().__endpointb.articleid);return f.___collection.getConnectedArticle(b)}),
f.parseConnection()):f.setupConnection(f.get("__endpointa"),f.get("__endpointb"));return f};e.Appacitive.Connection.multiDelete=function(b,a,c,d){if(!b)throw Error("Specify relationName");if(0<a.length){var h=new e.Appacitive.HttpRequest;h.url=e.Appacitive.config.apiBaseUrl+Appacitive.storage.urlFactory.connection.getMultideleteUrl(b);h.method="post";h.data={idlist:a};h.onSuccess=function(a){a&&"200"==a.code?c():(a=a||{},d(a||{message:"Server error",code:400}))};h.onError=function(a){a=a||{};d(a||
{message:"Server error",code:400})};e.Appacitive.http.send(h)}else c()}})(global);
